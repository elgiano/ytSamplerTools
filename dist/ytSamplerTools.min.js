...
"use strict";var __importDefault=this&&this.__importDefault||function(t){return t&&t.__esModule?t:{default:t}};Object.defineProperty(exports,"__esModule",{value:!0});const stream_1=require("stream"),sax_1=__importDefault(require("sax")),parse_time_1=require("./parse-time");class DashMPDParser extends stream_1.Writable{constructor(t){let e;super(),this._parser=sax_1.default.createStream(!1,{lowercase:!0}),this._parser.on("error",this.destroy.bind(this));let i,r,s,a,n,o,u,l,m=0,p=0,b=[],d=!1,c=!1;const h=e=>{const i={RepresentationID:t,Number:p,Time:m};return e.replace(/\$(\w+)\$/g,(t,e)=>`${i[e]}`)};this._parser.on("opentag",h=>{switch(h.name){case"mpd":m=h.attributes.availabilitystarttime?new Date(h.attributes.availabilitystarttime).getTime():0,o="dynamic"!==h.attributes.type;break;case"period":p=0,r=1e3,a=0,s=0,n=[],u=0,l=parse_time_1.durationStr(h.attributes.start)||0;break;case"segmentlist":p=parseInt(h.attributes.startnumber)||p,r=parseInt(h.attributes.timescale)||r,a=parseInt(h.attributes.duration)||a,s=parseInt(h.attributes.presentationtimeoffset)||s;break;case"segmenttemplate":i=h.attributes,p=parseInt(h.attributes.startnumber)||p,r=parseInt(h.attributes.timescale)||r;break;case"segmenttimeline":case"baseurl":e=h.name;break;case"s":b.push({duration:parseInt(h.attributes.d),repeat:parseInt(h.attributes.r),time:parseInt(h.attributes.t)});break;case"adaptationset":case"representation":u++,t||(t=h.attributes.id),(d=h.attributes.id===`${t}`)&&(l&&(m+=l),s&&(m-=s/r*1e3),this.emit("starttime",m));break;case"initialization":d&&this.emit("item",{url:n.filter(t=>!!t).join("")+h.attributes.sourceurl,seq:p,init:!0,duration:0});break;case"segmenturl":if(d){c=!0;let t=b.shift(),e=((null==t?void 0:t.duration)||a)/r*1e3;this.emit("item",{url:n.filter(t=>!!t).join("")+h.attributes.media,seq:p++,duration:e}),m+=e}}});const _=()=>{o&&this.emit("endlist"),d?this.emit("end"):this.destroy(Error(`Representation '${t}' not found`))};this._parser.on("closetag",t=>{switch(t){case"adaptationset":case"representation":if(u--,i&&b.length){c=!0,i.initialization&&this.emit("item",{url:n.filter(t=>!!t).join("")+h(i.initialization),seq:p,init:!0,duration:0});for(let{duration:t,repeat:e,time:s}of b){t=t/r*1e3,e=e||1,m=s||m;for(let r=0;r<e;r++)this.emit("item",{url:n.filter(t=>!!t).join("")+h(i.media),seq:p++,duration:t}),m+=t}}c&&(this.emit("endearly"),_(),this._parser.removeAllListeners(),this.removeAllListeners("finish"))}}),this._parser.on("text",t=>{"baseurl"===e&&(n[u]=t,e=null)}),this.on("finish",_)}_write(t,e,i){this._parser.write(t,e),i()}}exports.default=DashMPDParser;

},{"./parse-time":4,"sax":7,"stream":23}],2:[function(require,module,exports){
"use strict";var __importDefault=this&&this.__importDefault||function(e){return e&&e.__esModule?e:{default:e}};const stream_1=require("stream"),miniget_1=__importDefault(require("miniget")),m3u8_parser_1=__importDefault(require("./m3u8-parser")),dash_mpd_parser_1=__importDefault(require("./dash-mpd-parser")),queue_1=require("./queue"),parse_time_1=require("./parse-time"),URL="undefined"!=typeof window?window.URL:require("url").URL,supportedParsers={m3u8:m3u8_parser_1.default,"dash-mpd":dash_mpd_parser_1.default};let m3u8stream=(e,t={})=>{const r=new stream_1.PassThrough,n=t.chunkReadahead||3,s=t.liveBuffer||2e4,a=t.requestOptions,u=supportedParsers[t.parser||(/\.mpd$/.test(e)?"dash-mpd":"m3u8")];if(!u)throw TypeError(`parser '${t.parser}' not supported`);let i=0;void 0!==t.begin&&(i="string"==typeof t.begin?parse_time_1.humanStr(t.begin):Math.max(t.begin-s,0));const o=e=>{for(let t of["abort","request","response","redirect","retry","reconnect"])e.on(t,r.emit.bind(r,t))};let l;const d=new queue_1.Queue((e,t)=>{l=e;let n=0;e.on("data",e=>n+=e.length),e.pipe(r,{end:!1}),e.on("end",()=>t(null,n))},{concurrency:1});let m=0,p=0;const _=new queue_1.Queue((t,n)=>{let s=Object.assign({},a);t.range&&(s.headers=Object.assign({},s.headers,{Range:`bytes=${t.range.start}-${t.range.end}`}));let u=new URL(t.url,e).href,i=miniget_1.default(u,s);i.on("error",n),o(i),d.push(i,(e,s)=>{p+=+s,r.emit("progress",{num:++m,size:s,duration:t.duration,url:t.url},_.total,p),n(null)})},{concurrency:n}),h=e=>{w||(r.emit("error",e),r.end())};let c,f,g,q,b=!0,w=!1,y=!1;const D=e=>{if(l=null,e)h(e);else if(!b&&!w&&!y&&_.tasks.length+_.active<=c){let e=Math.max(0,f-(Date.now()-q));b=!0,g=setTimeout(M,e)}else!w&&!y||_.tasks.length||_.active||r.end()};let R,T,v=0;const M=()=>{q=Date.now(),(R=miniget_1.default(e,a)).on("error",h),o(R);const r=R.pipe(new u(t.id));r.on("starttime",e=>{v||(v=e,"string"==typeof t.begin&&i>=0&&(i+=v))}),r.on("endlist",()=>{y=!0}),r.on("endearly",R.unpipe.bind(R,r));let n=[];const l=e=>{if(!e.init){if(e.seq<=T)return;T=e.seq}i=e.time,_.push(e,D),n.push(e)};let d=[],m=0;r.on("item",e=>{let t=Object.assign({time:v},e);if(i<=t.time)l(t);else for(d.push(t),m+=t.duration;d.length>1&&m-d[0].duration>s;){const e=d.shift();m-=e.duration}v+=t.duration}),r.on("end",()=>{R=null,!n.length&&d.length&&d.forEach(e=>{l(e)}),c=Math.max(1,Math.ceil(.01*n.length)),f=n.reduce((e,t)=>t.duration+e,0),b=!1,D(null)})};return M(),r.end=(()=>{w=!0,d.die(),_.die(),clearTimeout(g),null==R||R.destroy(),null==l||l.destroy(),stream_1.PassThrough.prototype.end.call(r,null)}),r};m3u8stream.parseTimestamp=parse_time_1.humanStr,module.exports=m3u8stream;

},{"./dash-mpd-parser":1,"./m3u8-parser":3,"./parse-time":4,"./queue":5,"miniget":6,"stream":23,"url":60}],3:[function(require,module,exports){
"use strict";Object.defineProperty(exports,"__esModule",{value:!0});const stream_1=require("stream");class m3u8Parser extends stream_1.Writable{constructor(){super(),this._lastLine="",this._seq=0,this._nextItemDuration=null,this._nextItemRange=null,this._lastItemRangeEnd=0,this.on("finish",()=>{this._parseLine(this._lastLine),this.emit("end")})}_parseAttrList(t){let e,s={},i=/([A-Z0-9-]+)=(?:"([^"]*?)"|([^,]*?))/g;for(;null!==(e=i.exec(t));)s[e[1]]=e[2]||e[3];return s}_parseRange(t){if(!t)return null;let e=t.split("@"),s=e[1]?parseInt(e[1]):this._lastItemRangeEnd+1,i={start:s,end:s+parseInt(e[0])-1};return this._lastItemRangeEnd=i.end,i}_parseLine(t){let e=t.match(/^#(EXT[A-Z0-9-]+)(?::(.*))?/);if(e){const t=e[1],s=e[2]||"";switch(t){case"EXT-X-PROGRAM-DATE-TIME":this.emit("starttime",new Date(s).getTime());break;case"EXT-X-MEDIA-SEQUENCE":this._seq=parseInt(s);break;case"EXT-X-MAP":{let t=this._parseAttrList(s);if(!t.URI)return void this.destroy(new Error("`EXT-X-MAP` found without required attribute `URI`"));this.emit("item",{url:t.URI,seq:this._seq,init:!0,duration:0,range:this._parseRange(t.BYTERANGE)});break}case"EXT-X-BYTERANGE":this._nextItemRange=this._parseRange(s);break;case"EXTINF":this._nextItemDuration=Math.round(1e3*parseFloat(s.split(",")[0]));break;case"EXT-X-ENDLIST":this.emit("endlist")}}else!/^#/.test(t)&&t.trim()&&(this.emit("item",{url:t.trim(),seq:this._seq++,duration:this._nextItemDuration,range:this._nextItemRange}),this._nextItemRange=null)}_write(t,e,s){let i=t.toString("utf8").split("\n");this._lastLine&&(i[0]=this._lastLine+i[0]),i.forEach((t,e)=>{this.destroyed||(e<i.length-1?this._parseLine(t):this._lastLine=t)}),s()}}exports.default=m3u8Parser;

},{"stream":23}],4:[function(require,module,exports){
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.durationStr=exports.humanStr=void 0;const numberFormat=/^\d+$/,timeFormat=/^(?:(?:(\d+):)?(\d{1,2}):)?(\d{1,2})(?:\.(\d{3}))?$/,timeUnits={ms:1,s:1e3,m:6e4,h:36e5};exports.humanStr=(t=>{if("number"==typeof t)return t;if(numberFormat.test(t))return+t;const e=timeFormat.exec(t);if(e)return+(e[1]||0)*timeUnits.h+ +(e[2]||0)*timeUnits.m+ +e[3]*timeUnits.s+ +(e[4]||0);{let e=0;const r=/(-?\d+)(ms|s|m|h)/g;let n;for(;null!==(n=r.exec(t));)e+=+n[1]*timeUnits[n[2]];return e}}),exports.durationStr=(t=>{let e=0;const r=/(\d+(?:\.\d+)?)(S|M|H)/g;let n;for(;null!==(n=r.exec(t));)e+=+n[1]*timeUnits[n[2].toLowerCase()];return e});

},{}],5:[function(require,module,exports){
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.Queue=void 0;class Queue{constructor(t,s={}){this._worker=t,this._concurrency=s.concurrency||1,this.tasks=[],this.total=0,this.active=0}push(t,s){this.tasks.push({item:t,callback:s}),this.total++,this._next()}_next(){if(this.active>=this._concurrency||!this.tasks.length)return;const{item:t,callback:s}=this.tasks.shift();let e=!1;this.active++,this._worker(t,(t,i)=>{e||(this.active--,e=!0,null==s||s(t,i),this._next())})}die(){this.tasks=[]}}exports.Queue=Queue;

},{}],6:[function(require,module,exports){
(function (process){(function (){
"use strict";var __awaiter=this&&this.__awaiter||function(e,t,r,n){return new(r||(r=Promise))(function(o,s){function i(e){try{d(n.next(e))}catch(e){s(e)}}function a(e){try{d(n.throw(e))}catch(e){s(e)}}function d(e){var t;e.done?o(e.value):(t=e.value,t instanceof r?t:new r(function(e){e(t)})).then(i,a)}d((n=n.apply(e,t||[])).next())})},__importDefault=this&&this.__importDefault||function(e){return e&&e.__esModule?e:{default:e}};const http_1=__importDefault(require("http")),https_1=__importDefault(require("https")),url_1=require("url"),stream_1=require("stream"),httpLibs={"http:":http_1.default,"https:":https_1.default},redirectStatusCodes=new Set([301,302,303,307,308]),retryStatusCodes=new Set([429,503]),requestEvents=["connect","continue","information","socket","timeout","upgrade"],responseEvents=["aborted"];function Miniget(e,t={}){var r;const n=Object.assign({},Miniget.defaultOptions,t),o=new stream_1.PassThrough({highWaterMark:n.highWaterMark});let s,i,a;o.destroyed=o.aborted=!1;let d,c,u,l=0,f=0,m=0,h=!1,p=0,g=0;if(null===(r=n.headers)||void 0===r?void 0:r.Range){let e=/bytes=(\d+)-(\d+)?/.exec(n.headers.Range+"");e&&(p=parseInt(e[1],10),u=parseInt(e[2],10))}n.acceptEncoding&&(n.headers=Object.assign({"Accept-Encoding":Object.keys(n.acceptEncoding).join(", ")},n.headers));const v=e=>"HEAD"!=t.method&&!(()=>!h||g==c)()&&m++<n.maxReconnects&&((e=>{a=null,f=0;let t=n.backoff.inc,r=Math.min(t,n.backoff.max);d=setTimeout(_,r),o.emit("reconnect",m,e)})(e),!0),y=e=>{if(o.destroyed)return!1;if((()=>a&&0<g)())return v(e.err);if((!e.statusCode||"ENOTFOUND"===e.err.message)&&f++<n.maxRetries){let t=e.retryAfter||Math.min(f*n.backoff.inc,n.backoff.max);return d=setTimeout(_,t),o.emit("retry",f,e.err),!0}return!1},b=(e,t)=>{for(let r of t)e.on(r,o.emit.bind(o,r))},_=()=>{let t,r;try{t=url_1.parse(e),r=httpLibs[t.protocol]}catch(e){}if(!r)return void o.emit("error",new Miniget.MinigetError("Invalid URL: "+e));if(Object.assign(t,n),h&&g>0){let e=g+p,r=u||"";t.headers=Object.assign({},t.headers,{Range:`bytes=${e}-${r}`})}if(n.transform){try{t=n.transform(t)}catch(e){return void o.emit("error",e)}if((!t||t.protocol)&&!(r=httpLibs[null==t?void 0:t.protocol]))return void o.emit("error",new Miniget.MinigetError("Invalid URL object from `transform` function"))}const d=(e,t)=>{m(),y({err:e,statusCode:t})?s.removeListener("close",f):o.emit("error",e)},f=()=>{m(),y({})},m=()=>{s.removeListener("error",d),s.removeListener("close",f),null==i||i.removeListener("data",x),null==a||a.removeListener("end",C),null==a||a.removeListener("error",d),null==i||i.removeListener("error",d)},x=e=>{g+=e.length},C=()=>{m(),v()||o.end()};(s=r.get(t,t=>{if(!o.destroyed){if(redirectStatusCodes.has(t.statusCode))return l++>=n.maxRedirects?o.emit("error",new Miniget.MinigetError("Too many redirects")):(e=t.headers.location,setTimeout(_,t.headers["retry-after"]?1e3*parseInt(t.headers["retry-after"],10):0),o.emit("redirect",e)),m();if(retryStatusCodes.has(t.statusCode)){if(!y({retryAfter:parseInt(t.headers["retry-after"],10)})){let e=new Miniget.MinigetError("Status code: "+t.statusCode,t.statusCode);o.emit("error",e)}return m()}if(t.statusCode<200||400<=t.statusCode){let e=new Miniget.MinigetError("Status code: "+t.statusCode,t.statusCode);return t.statusCode>=500?d(e,t.statusCode):o.emit("error",e),m()}if(a=t,n.acceptEncoding&&t.headers["content-encoding"])for(let e of t.headers["content-encoding"].split(", ").reverse()){let t=n.acceptEncoding[e];null!=t&&(a=a.pipe(t())).on("error",d)}c||(c=parseInt(t.headers["content-length"]+"",10),h="bytes"===t.headers["accept-ranges"]&&c>0&&n.maxReconnects>0),t.on("data",x),a.on("end",C),a.pipe(o,{end:!h}),i=t,o.emit("response",t),t.on("error",d),b(t,responseEvents)}})).on("error",d),s.on("close",f),b(s,requestEvents),o.destroyed&&E(M),o.emit("request",s)};let M;o.abort=(e=>{console.warn("`MinigetStream#abort()` has been deprecated in favor of `MinigetStream#destroy()`"),o.aborted=!0,o.emit("abort"),o.destroy(e)});const E=e=>{s.destroy(e),null==a||a.unpipe(o),null==a||a.destroy(),clearTimeout(d)};return o._destroy=(e=>{o.destroyed=!0,s?E(e):M=e}),o.text=(()=>__awaiter(this,void 0,void 0,function*(){return new Promise((e,t)=>{let r="";o.setEncoding("utf8"),o.on("data",e=>r+=e),o.on("end",()=>e(r)),o.on("error",t)})})),process.nextTick(_),o}Miniget.MinigetError=class extends Error{constructor(e,t){super(e),this.statusCode=t}},Miniget.defaultOptions={maxRedirects:10,maxRetries:2,maxReconnects:0,backoff:{inc:100,max:1e4}},module.exports=Miniget;
...
},{"buffer":10,"stream":23,"string_decoder":58}],23:[function(require,module,exports){
...
},{"./_stream_readable":45,"./_stream_writable":47,"_process":16,"inherits":15}],44:[function(require,module,exports){
"use strict";module.exports=PassThrough;var Transform=require("./_stream_transform");function PassThrough(r){if(!(this instanceof PassThrough))return new PassThrough(r);Transform.call(this,r)}require("inherits")(PassThrough,Transform),PassThrough.prototype._transform=function(r,s,o){o(null,r)};

},{"./_stream_transform":46,"inherits":15}],45:[function(require,module,exports){
(function (process,global){(function (){
"use strict";var Duplex;module.exports=Readable,Readable.ReadableState=ReadableState;var EE=require("events").EventEmitter,EElistenerCount=function(e,t){return e.listeners(t).length},Stream=require("./internal/streams/stream"),Buffer=require("buffer").Buffer,OurUint8Array=global.Uint8Array||function(){};function _uint8ArrayToBuffer(e){return Buffer.from(e)}function _isUint8Array(e){return Buffer.isBuffer(e)||e instanceof OurUint8Array}var debug,debugUtil=require("util");debug=debugUtil&&debugUtil.debuglog?debugUtil.debuglog("stream"):function(){};var StringDecoder,createReadableStreamAsyncIterator,from,BufferList=require("./internal/streams/buffer_list"),destroyImpl=require("./internal/streams/destroy"),_require=require("./internal/streams/state"),getHighWaterMark=_require.getHighWaterMark,_require$codes=require("../errors").codes,ERR_INVALID_ARG_TYPE=_require$codes.ERR_INVALID_ARG_TYPE,ERR_STREAM_PUSH_AFTER_EOF=_require$codes.ERR_STREAM_PUSH_AFTER_EOF,ERR_METHOD_NOT_IMPLEMENTED=_require$codes.ERR_METHOD_NOT_IMPLEMENTED,ERR_STREAM_UNSHIFT_AFTER_END_EVENT=_require$codes.ERR_STREAM_UNSHIFT_AFTER_END_EVENT;require("inherits")(Readable,Stream);var errorOrDestroy=destroyImpl.errorOrDestroy,kProxyEvents=["error","close","destroy","pause","resume"];function prependListener(e,t,r){if("function"==typeof e.prependListener)return e.prependListener(t,r);e._events&&e._events[t]?Array.isArray(e._events[t])?e._events[t].unshift(r):e._events[t]=[r,e._events[t]]:e.on(t,r)}function ReadableState(e,t,r){Duplex=Duplex||require("./_stream_duplex"),e=e||{},"boolean"!=typeof r&&(r=t instanceof Duplex),this.objectMode=!!e.objectMode,r&&(this.objectMode=this.objectMode||!!e.readableObjectMode),this.highWaterMark=getHighWaterMark(this,e,"readableHighWaterMark",r),this.buffer=new BufferList,this.length=0,this.pipes=null,this.pipesCount=0,this.flowing=null,this.ended=!1,this.endEmitted=!1,this.reading=!1,this.sync=!0,this.needReadable=!1,this.emittedReadable=!1,this.readableListening=!1,this.resumeScheduled=!1,this.paused=!0,this.emitClose=!1!==e.emitClose,this.autoDestroy=!!e.autoDestroy,this.destroyed=!1,this.defaultEncoding=e.defaultEncoding||"utf8",this.awaitDrain=0,this.readingMore=!1,this.decoder=null,this.encoding=null,e.encoding&&(StringDecoder||(StringDecoder=require("string_decoder/").StringDecoder),this.decoder=new StringDecoder(e.encoding),this.encoding=e.encoding)}function Readable(e){if(Duplex=Duplex||require("./_stream_duplex"),!(this instanceof Readable))return new Readable(e);var t=this instanceof Duplex;this._readableState=new ReadableState(e,this,t),this.readable=!0,e&&("function"==typeof e.read&&(this._read=e.read),"function"==typeof e.destroy&&(this._destroy=e.destroy)),Stream.call(this)}function readableAddChunk(e,t,r,a,n){debug("readableAddChunk",t);var i,d=e._readableState;if(null===t)d.reading=!1,onEofChunk(e,d);else if(n||(i=chunkInvalid(d,t)),i)errorOrDestroy(e,i);else if(d.objectMode||t&&t.length>0)if("string"==typeof t||d.objectMode||Object.getPrototypeOf(t)===Buffer.prototype||(t=_uint8ArrayToBuffer(t)),a)d.endEmitted?errorOrDestroy(e,new ERR_STREAM_UNSHIFT_AFTER_END_EVENT):addChunk(e,d,t,!0);else if(d.ended)errorOrDestroy(e,new ERR_STREAM_PUSH_AFTER_EOF);else{if(d.destroyed)return!1;d.reading=!1,d.decoder&&!r?(t=d.decoder.write(t),d.objectMode||0!==t.length?addChunk(e,d,t,!1):maybeReadMore(e,d)):addChunk(e,d,t,!1)}else a||(d.reading=!1,maybeReadMore(e,d));return!d.ended&&(d.length<d.highWaterMark||0===d.length)}function addChunk(e,t,r,a){t.flowing&&0===t.length&&!t.sync?(t.awaitDrain=0,e.emit("data",r)):(t.length+=t.objectMode?1:r.length,a?t.buffer.unshift(r):t.buffer.push(r),t.needReadable&&emitReadable(e)),maybeReadMore(e,t)}function chunkInvalid(e,t){var r;return _isUint8Array(t)||"string"==typeof t||void 0===t||e.objectMode||(r=new ERR_INVALID_ARG_TYPE("chunk",["string","Buffer","Uint8Array"],t)),r}Object.defineProperty(Readable.prototype,"destroyed",{enumerable:!1,get:function(){return void 0!==this._readableState&&this._readableState.destroyed},set:function(e){this._readableState&&(this._readableState.destroyed=e)}}),Readable.prototype.destroy=destroyImpl.destroy,Readable.prototype._undestroy=destroyImpl.undestroy,Readable.prototype._destroy=function(e,t){t(e)},Readable.prototype.push=function(e,t){var r,a=this._readableState;return a.objectMode?r=!0:"string"==typeof e&&((t=t||a.defaultEncoding)!==a.encoding&&(e=Buffer.from(e,t),t=""),r=!0),readableAddChunk(this,e,t,!1,r)},Readable.prototype.unshift=function(e){return readableAddChunk(this,e,null,!0,!1)},Readable.prototype.isPaused=function(){return!1===this._readableState.flowing},Readable.prototype.setEncoding=function(e){StringDecoder||(StringDecoder=require("string_decoder/").StringDecoder);var t=new StringDecoder(e);this._readableState.decoder=t,this._readableState.encoding=this._readableState.decoder.encoding;for(var r=this._readableState.buffer.head,a="";null!==r;)a+=t.write(r.data),r=r.next;return this._readableState.buffer.clear(),""!==a&&this._readableState.buffer.push(a),this._readableState.length=a.length,this};var MAX_HWM=1073741824;function computeNewHighWaterMark(e){return e>=MAX_HWM?e=MAX_HWM:(e--,e|=e>>>1,e|=e>>>2,e|=e>>>4,e|=e>>>8,e|=e>>>16,e++),e}function howMuchToRead(e,t){return e<=0||0===t.length&&t.ended?0:t.objectMode?1:e!=e?t.flowing&&t.length?t.buffer.head.data.length:t.length:(e>t.highWaterMark&&(t.highWaterMark=computeNewHighWaterMark(e)),e<=t.length?e:t.ended?t.length:(t.needReadable=!0,0))}function onEofChunk(e,t){if(debug("onEofChunk"),!t.ended){if(t.decoder){var r=t.decoder.end();r&&r.length&&(t.buffer.push(r),t.length+=t.objectMode?1:r.length)}t.ended=!0,t.sync?emitReadable(e):(t.needReadable=!1,t.emittedReadable||(t.emittedReadable=!0,emitReadable_(e)))}}function emitReadable(e){var t=e._readableState;debug("emitReadable",t.needReadable,t.emittedReadable),t.needReadable=!1,t.emittedReadable||(debug("emitReadable",t.flowing),t.emittedReadable=!0,process.nextTick(emitReadable_,e))}function emitReadable_(e){var t=e._readableState;debug("emitReadable_",t.destroyed,t.length,t.ended),t.destroyed||!t.length&&!t.ended||(e.emit("readable"),t.emittedReadable=!1),t.needReadable=!t.flowing&&!t.ended&&t.length<=t.highWaterMark,flow(e)}function maybeReadMore(e,t){t.readingMore||(t.readingMore=!0,process.nextTick(maybeReadMore_,e,t))}function maybeReadMore_(e,t){for(;!t.reading&&!t.ended&&(t.length<t.highWaterMark||t.flowing&&0===t.length);){var r=t.length;if(debug("maybeReadMore read 0"),e.read(0),r===t.length)break}t.readingMore=!1}function pipeOnDrain(e){return function(){var t=e._readableState;debug("pipeOnDrain",t.awaitDrain),t.awaitDrain&&t.awaitDrain--,0===t.awaitDrain&&EElistenerCount(e,"data")&&(t.flowing=!0,flow(e))}}function updateReadableListening(e){var t=e._readableState;t.readableListening=e.listenerCount("readable")>0,t.resumeScheduled&&!t.paused?t.flowing=!0:e.listenerCount("data")>0&&e.resume()}function nReadingNextTick(e){debug("readable nexttick read 0"),e.read(0)}function resume(e,t){t.resumeScheduled||(t.resumeScheduled=!0,process.nextTick(resume_,e,t))}function resume_(e,t){debug("resume",t.reading),t.reading||e.read(0),t.resumeScheduled=!1,e.emit("resume"),flow(e),t.flowing&&!t.reading&&e.read(0)}function flow(e){var t=e._readableState;for(debug("flow",t.flowing);t.flowing&&null!==e.read(););}function fromList(e,t){return 0===t.length?null:(t.objectMode?r=t.buffer.shift():!e||e>=t.length?(r=t.decoder?t.buffer.join(""):1===t.buffer.length?t.buffer.first():t.buffer.concat(t.length),t.buffer.clear()):r=t.buffer.consume(e,t.decoder),r);var r}function endReadable(e){var t=e._readableState;debug("endReadable",t.endEmitted),t.endEmitted||(t.ended=!0,process.nextTick(endReadableNT,t,e))}function endReadableNT(e,t){if(debug("endReadableNT",e.endEmitted,e.length),!e.endEmitted&&0===e.length&&(e.endEmitted=!0,t.readable=!1,t.emit("end"),e.autoDestroy)){var r=t._writableState;(!r||r.autoDestroy&&r.finished)&&t.destroy()}}function indexOf(e,t){for(var r=0,a=e.length;r<a;r++)if(e[r]===t)return r;return-1}Readable.prototype.read=function(e){debug("read",e),e=parseInt(e,10);var t=this._readableState,r=e;if(0!==e&&(t.emittedReadable=!1),0===e&&t.needReadable&&((0!==t.highWaterMark?t.length>=t.highWaterMark:t.length>0)||t.ended))return debug("read: emitReadable",t.length,t.ended),0===t.length&&t.ended?endReadable(this):emitReadable(this),null;if(0===(e=howMuchToRead(e,t))&&t.ended)return 0===t.length&&endReadable(this),null;var a,n=t.needReadable;return debug("need readable",n),(0===t.length||t.length-e<t.highWaterMark)&&debug("length less than watermark",n=!0),t.ended||t.reading?debug("reading or ended",n=!1):n&&(debug("do read"),t.reading=!0,t.sync=!0,0===t.length&&(t.needReadable=!0),this._read(t.highWaterMark),t.sync=!1,t.reading||(e=howMuchToRead(r,t))),null===(a=e>0?fromList(e,t):null)?(t.needReadable=t.length<=t.highWaterMark,e=0):(t.length-=e,t.awaitDrain=0),0===t.length&&(t.ended||(t.needReadable=!0),r!==e&&t.ended&&endReadable(this)),null!==a&&this.emit("data",a),a},Readable.prototype._read=function(e){errorOrDestroy(this,new ERR_METHOD_NOT_IMPLEMENTED("_read()"))},Readable.prototype.pipe=function(e,t){var r=this,a=this._readableState;switch(a.pipesCount){case 0:a.pipes=e;break;case 1:a.pipes=[a.pipes,e];break;default:a.pipes.push(e)}a.pipesCount+=1,debug("pipe count=%d opts=%j",a.pipesCount,t);var n=(!t||!1!==t.end)&&e!==process.stdout&&e!==process.stderr?d:f;function i(t,n){debug("onunpipe"),t===r&&n&&!1===n.hasUnpiped&&(n.hasUnpiped=!0,debug("cleanup"),e.removeListener("close",b),e.removeListener("finish",p),e.removeListener("drain",o),e.removeListener("error",u),e.removeListener("unpipe",i),r.removeListener("end",d),r.removeListener("end",f),r.removeListener("data",l),s=!0,!a.awaitDrain||e._writableState&&!e._writableState.needDrain||o())}function d(){debug("onend"),e.end()}a.endEmitted?process.nextTick(n):r.once("end",n),e.on("unpipe",i);var o=pipeOnDrain(r);e.on("drain",o);var s=!1;function l(t){debug("ondata");var n=e.write(t);debug("dest.write",n),!1===n&&((1===a.pipesCount&&a.pipes===e||a.pipesCount>1&&-1!==indexOf(a.pipes,e))&&!s&&(debug("false write response, pause",a.awaitDrain),a.awaitDrain++),r.pause())}function u(t){debug("onerror",t),f(),e.removeListener("error",u),0===EElistenerCount(e,"error")&&errorOrDestroy(e,t)}function b(){e.removeListener("finish",p),f()}function p(){debug("onfinish"),e.removeListener("close",b),f()}function f(){debug("unpipe"),r.unpipe(e)}return r.on("data",l),prependListener(e,"error",u),e.once("close",b),e.once("finish",p),e.emit("pipe",r),a.flowing||(debug("pipe resume"),r.resume()),e},Readable.prototype.unpipe=function(e){var t=this._readableState,r={hasUnpiped:!1};if(0===t.pipesCount)return this;if(1===t.pipesCount)return e&&e!==t.pipes?this:(e||(e=t.pipes),t.pipes=null,t.pipesCount=0,t.flowing=!1,e&&e.emit("unpipe",this,r),this);if(!e){var a=t.pipes,n=t.pipesCount;t.pipes=null,t.pipesCount=0,t.flowing=!1;for(var i=0;i<n;i++)a[i].emit("unpipe",this,{hasUnpiped:!1});return this}var d=indexOf(t.pipes,e);return-1===d?this:(t.pipes.splice(d,1),t.pipesCount-=1,1===t.pipesCount&&(t.pipes=t.pipes[0]),e.emit("unpipe",this,r),this)},Readable.prototype.on=function(e,t){var r=Stream.prototype.on.call(this,e,t),a=this._readableState;return"data"===e?(a.readableListening=this.listenerCount("readable")>0,!1!==a.flowing&&this.resume()):"readable"===e&&(a.endEmitted||a.readableListening||(a.readableListening=a.needReadable=!0,a.flowing=!1,a.emittedReadable=!1,debug("on readable",a.length,a.reading),a.length?emitReadable(this):a.reading||process.nextTick(nReadingNextTick,this))),r},Readable.prototype.addListener=Readable.prototype.on,Readable.prototype.removeListener=function(e,t){var r=Stream.prototype.removeListener.call(this,e,t);return"readable"===e&&process.nextTick(updateReadableListening,this),r},Readable.prototype.removeAllListeners=function(e){var t=Stream.prototype.removeAllListeners.apply(this,arguments);return"readable"!==e&&void 0!==e||process.nextTick(updateReadableListening,this),t},Readable.prototype.resume=function(){var e=this._readableState;return e.flowing||(debug("resume"),e.flowing=!e.readableListening,resume(this,e)),e.paused=!1,this},Readable.prototype.pause=function(){return debug("call pause flowing=%j",this._readableState.flowing),!1!==this._readableState.flowing&&(debug("pause"),this._readableState.flowing=!1,this.emit("pause")),this._readableState.paused=!0,this},Readable.prototype.wrap=function(e){var t=this,r=this._readableState,a=!1;for(var n in e.on("end",function(){if(debug("wrapped end"),r.decoder&&!r.ended){var e=r.decoder.end();e&&e.length&&t.push(e)}t.push(null)}),e.on("data",function(n){(debug("wrapped data"),r.decoder&&(n=r.decoder.write(n)),r.objectMode&&null==n)||(r.objectMode||n&&n.length)&&(t.push(n)||(a=!0,e.pause()))}),e)void 0===this[n]&&"function"==typeof e[n]&&(this[n]=function(t){return function(){return e[t].apply(e,arguments)}}(n));for(var i=0;i<kProxyEvents.length;i++)e.on(kProxyEvents[i],this.emit.bind(this,kProxyEvents[i]));return this._read=function(t){debug("wrapped _read",t),a&&(a=!1,e.resume())},this},"function"==typeof Symbol&&(Readable.prototype[Symbol.asyncIterator]=function(){return void 0===createReadableStreamAsyncIterator&&(createReadableStreamAsyncIterator=require("./internal/streams/async_iterator")),createReadableStreamAsyncIterator(this)}),Object.defineProperty(Readable.prototype,"readableHighWaterMark",{enumerable:!1,get:function(){return this._readableState.highWaterMark}}),Object.defineProperty(Readable.prototype,"readableBuffer",{enumerable:!1,get:function(){return this._readableState&&this._readableState.buffer}}),Object.defineProperty(Readable.prototype,"readableFlowing",{enumerable:!1,get:function(){return this._readableState.flowing},set:function(e){this._readableState&&(this._readableState.flowing=e)}}),Readable._fromList=fromList,Object.defineProperty(Readable.prototype,"readableLength",{enumerable:!1,get:function(){return this._readableState.length}}),"function"==typeof Symbol&&(Readable.from=function(e,t){return void 0===from&&(from=require("./internal/streams/from")),from(Readable,e,t)});

}).call(this)}).call(this,require('_process'),typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"../errors":42,"./_stream_duplex":43,"./internal/streams/async_iterator":48,"./internal/streams/buffer_list":49,"./internal/streams/destroy":50,"./internal/streams/from":52,"./internal/streams/state":54,"./internal/streams/stream":55,"_process":16,"buffer":10,"events":12,"inherits":15,"string_decoder/":58,"util":9}],46:[function(require,module,exports){
"use strict";module.exports=Transform;var _require$codes=require("../errors").codes,ERR_METHOD_NOT_IMPLEMENTED=_require$codes.ERR_METHOD_NOT_IMPLEMENTED,ERR_MULTIPLE_CALLBACK=_require$codes.ERR_MULTIPLE_CALLBACK,ERR_TRANSFORM_ALREADY_TRANSFORMING=_require$codes.ERR_TRANSFORM_ALREADY_TRANSFORMING,ERR_TRANSFORM_WITH_LENGTH_0=_require$codes.ERR_TRANSFORM_WITH_LENGTH_0,Duplex=require("./_stream_duplex");function afterTransform(r,e){var t=this._transformState;t.transforming=!1;var n=t.writecb;if(null===n)return this.emit("error",new ERR_MULTIPLE_CALLBACK);t.writechunk=null,t.writecb=null,null!=e&&this.push(e),n(r);var i=this._readableState;i.reading=!1,(i.needReadable||i.length<i.highWaterMark)&&this._read(i.highWaterMark)}function Transform(r){if(!(this instanceof Transform))return new Transform(r);Duplex.call(this,r),this._transformState={afterTransform:afterTransform.bind(this),needTransform:!1,transforming:!1,writecb:null,writechunk:null,writeencoding:null},this._readableState.needReadable=!0,this._readableState.sync=!1,r&&("function"==typeof r.transform&&(this._transform=r.transform),"function"==typeof r.flush&&(this._flush=r.flush)),this.on("prefinish",prefinish)}function prefinish(){var r=this;"function"!=typeof this._flush||this._readableState.destroyed?done(this,null,null):this._flush(function(e,t){done(r,e,t)})}function done(r,e,t){if(e)return r.emit("error",e);if(null!=t&&r.push(t),r._writableState.length)throw new ERR_TRANSFORM_WITH_LENGTH_0;if(r._transformState.transforming)throw new ERR_TRANSFORM_ALREADY_TRANSFORMING;return r.push(null)}require("inherits")(Transform,Duplex),Transform.prototype.push=function(r,e){return this._transformState.needTransform=!1,Duplex.prototype.push.call(this,r,e)},Transform.prototype._transform=function(r,e,t){t(new ERR_METHOD_NOT_IMPLEMENTED("_transform()"))},Transform.prototype._write=function(r,e,t){var n=this._transformState;if(n.writecb=t,n.writechunk=r,n.writeencoding=e,!n.transforming){var i=this._readableState;(n.needTransform||i.needReadable||i.length<i.highWaterMark)&&this._read(i.highWaterMark)}},Transform.prototype._read=function(r){var e=this._transformState;null===e.writechunk||e.transforming?e.needTransform=!0:(e.transforming=!0,this._transform(e.writechunk,e.writeencoding,e.afterTransform))},Transform.prototype._destroy=function(r,e){Duplex.prototype._destroy.call(this,r,function(r){e(r)})};

},{"../errors":42,"./_stream_duplex":43,"inherits":15}],47:[function(require,module,exports){
(function (process,global){(function (){
"use strict";function WriteReq(e,t,r){this.chunk=e,this.encoding=t,this.callback=r,this.next=null}function CorkedRequest(e){var t=this;this.next=null,this.entry=null,this.finish=function(){onCorkedFinish(t,e)}}var Duplex;module.exports=Writable,Writable.WritableState=WritableState;var internalUtil={deprecate:require("util-deprecate")},Stream=require("./internal/streams/stream"),Buffer=require("buffer").Buffer,OurUint8Array=global.Uint8Array||function(){};function _uint8ArrayToBuffer(e){return Buffer.from(e)}function _isUint8Array(e){return Buffer.isBuffer(e)||e instanceof OurUint8Array}var realHasInstance,destroyImpl=require("./internal/streams/destroy"),_require=require("./internal/streams/state"),getHighWaterMark=_require.getHighWaterMark,_require$codes=require("../errors").codes,ERR_INVALID_ARG_TYPE=_require$codes.ERR_INVALID_ARG_TYPE,ERR_METHOD_NOT_IMPLEMENTED=_require$codes.ERR_METHOD_NOT_IMPLEMENTED,ERR_MULTIPLE_CALLBACK=_require$codes.ERR_MULTIPLE_CALLBACK,ERR_STREAM_CANNOT_PIPE=_require$codes.ERR_STREAM_CANNOT_PIPE,ERR_STREAM_DESTROYED=_require$codes.ERR_STREAM_DESTROYED,ERR_STREAM_NULL_VALUES=_require$codes.ERR_STREAM_NULL_VALUES,ERR_STREAM_WRITE_AFTER_END=_require$codes.ERR_STREAM_WRITE_AFTER_END,ERR_UNKNOWN_ENCODING=_require$codes.ERR_UNKNOWN_ENCODING,errorOrDestroy=destroyImpl.errorOrDestroy;function nop(){}function WritableState(e,t,r){Duplex=Duplex||require("./_stream_duplex"),e=e||{},"boolean"!=typeof r&&(r=t instanceof Duplex),this.objectMode=!!e.objectMode,r&&(this.objectMode=this.objectMode||!!e.writableObjectMode),this.highWaterMark=getHighWaterMark(this,e,"writableHighWaterMark",r),this.finalCalled=!1,this.needDrain=!1,this.ending=!1,this.ended=!1,this.finished=!1,this.destroyed=!1;var i=!1===e.decodeStrings;this.decodeStrings=!i,this.defaultEncoding=e.defaultEncoding||"utf8",this.length=0,this.writing=!1,this.corked=0,this.sync=!0,this.bufferProcessing=!1,this.onwrite=function(e){onwrite(t,e)},this.writecb=null,this.writelen=0,this.bufferedRequest=null,this.lastBufferedRequest=null,this.pendingcb=0,this.prefinished=!1,this.errorEmitted=!1,this.emitClose=!1!==e.emitClose,this.autoDestroy=!!e.autoDestroy,this.bufferedRequestCount=0,this.corkedRequestsFree=new CorkedRequest(this)}function Writable(e){var t=this instanceof(Duplex=Duplex||require("./_stream_duplex"));if(!t&&!realHasInstance.call(Writable,this))return new Writable(e);this._writableState=new WritableState(e,this,t),this.writable=!0,e&&("function"==typeof e.write&&(this._write=e.write),"function"==typeof e.writev&&(this._writev=e.writev),"function"==typeof e.destroy&&(this._destroy=e.destroy),"function"==typeof e.final&&(this._final=e.final)),Stream.call(this)}function writeAfterEnd(e,t){var r=new ERR_STREAM_WRITE_AFTER_END;errorOrDestroy(e,r),process.nextTick(t,r)}function validChunk(e,t,r,i){var n;return null===r?n=new ERR_STREAM_NULL_VALUES:"string"==typeof r||t.objectMode||(n=new ERR_INVALID_ARG_TYPE("chunk",["string","Buffer"],r)),!n||(errorOrDestroy(e,n),process.nextTick(i,n),!1)}function decodeChunk(e,t,r){return e.objectMode||!1===e.decodeStrings||"string"!=typeof t||(t=Buffer.from(t,r)),t}function writeOrBuffer(e,t,r,i,n,o){if(!r){var s=decodeChunk(t,i,n);i!==s&&(r=!0,n="buffer",i=s)}var a=t.objectMode?1:i.length;t.length+=a;var u=t.length<t.highWaterMark;if(u||(t.needDrain=!0),t.writing||t.corked){var f=t.lastBufferedRequest;t.lastBufferedRequest={chunk:i,encoding:n,isBuf:r,callback:o,next:null},f?f.next=t.lastBufferedRequest:t.bufferedRequest=t.lastBufferedRequest,t.bufferedRequestCount+=1}else doWrite(e,t,!1,a,i,n,o);return u}function doWrite(e,t,r,i,n,o,s){t.writelen=i,t.writecb=s,t.writing=!0,t.sync=!0,t.destroyed?t.onwrite(new ERR_STREAM_DESTROYED("write")):r?e._writev(n,t.onwrite):e._write(n,o,t.onwrite),t.sync=!1}function onwriteError(e,t,r,i,n){--t.pendingcb,r?(process.nextTick(n,i),process.nextTick(finishMaybe,e,t),e._writableState.errorEmitted=!0,errorOrDestroy(e,i)):(n(i),e._writableState.errorEmitted=!0,errorOrDestroy(e,i),finishMaybe(e,t))}function onwriteStateUpdate(e){e.writing=!1,e.writecb=null,e.length-=e.writelen,e.writelen=0}function onwrite(e,t){var r=e._writableState,i=r.sync,n=r.writecb;if("function"!=typeof n)throw new ERR_MULTIPLE_CALLBACK;if(onwriteStateUpdate(r),t)onwriteError(e,r,i,t,n);else{var o=needFinish(r)||e.destroyed;o||r.corked||r.bufferProcessing||!r.bufferedRequest||clearBuffer(e,r),i?process.nextTick(afterWrite,e,r,o,n):afterWrite(e,r,o,n)}}function afterWrite(e,t,r,i){r||onwriteDrain(e,t),t.pendingcb--,i(),finishMaybe(e,t)}function onwriteDrain(e,t){0===t.length&&t.needDrain&&(t.needDrain=!1,e.emit("drain"))}function clearBuffer(e,t){t.bufferProcessing=!0;var r=t.bufferedRequest;if(e._writev&&r&&r.next){var i=t.bufferedRequestCount,n=new Array(i),o=t.corkedRequestsFree;o.entry=r;for(var s=0,a=!0;r;)n[s]=r,r.isBuf||(a=!1),r=r.next,s+=1;n.allBuffers=a,doWrite(e,t,!0,t.length,n,"",o.finish),t.pendingcb++,t.lastBufferedRequest=null,o.next?(t.corkedRequestsFree=o.next,o.next=null):t.corkedRequestsFree=new CorkedRequest(t),t.bufferedRequestCount=0}else{for(;r;){var u=r.chunk,f=r.encoding,l=r.callback;if(doWrite(e,t,!1,t.objectMode?1:u.length,u,f,l),r=r.next,t.bufferedRequestCount--,t.writing)break}null===r&&(t.lastBufferedRequest=null)}t.bufferedRequest=r,t.bufferProcessing=!1}function needFinish(e){return e.ending&&0===e.length&&null===e.bufferedRequest&&!e.finished&&!e.writing}function callFinal(e,t){e._final(function(r){t.pendingcb--,r&&errorOrDestroy(e,r),t.prefinished=!0,e.emit("prefinish"),finishMaybe(e,t)})}function prefinish(e,t){t.prefinished||t.finalCalled||("function"!=typeof e._final||t.destroyed?(t.prefinished=!0,e.emit("prefinish")):(t.pendingcb++,t.finalCalled=!0,process.nextTick(callFinal,e,t)))}function finishMaybe(e,t){var r=needFinish(t);if(r&&(prefinish(e,t),0===t.pendingcb&&(t.finished=!0,e.emit("finish"),t.autoDestroy))){var i=e._readableState;(!i||i.autoDestroy&&i.endEmitted)&&e.destroy()}return r}function endWritable(e,t,r){t.ending=!0,finishMaybe(e,t),r&&(t.finished?process.nextTick(r):e.once("finish",r)),t.ended=!0,e.writable=!1}function onCorkedFinish(e,t,r){var i=e.entry;for(e.entry=null;i;){var n=i.callback;t.pendingcb--,n(r),i=i.next}t.corkedRequestsFree.next=e}require("inherits")(Writable,Stream),WritableState.prototype.getBuffer=function(){for(var e=this.bufferedRequest,t=[];e;)t.push(e),e=e.next;return t},function(){try{Object.defineProperty(WritableState.prototype,"buffer",{get:internalUtil.deprecate(function(){return this.getBuffer()},"_writableState.buffer is deprecated. Use _writableState.getBuffer instead.","DEP0003")})}catch(e){}}(),"function"==typeof Symbol&&Symbol.hasInstance&&"function"==typeof Function.prototype[Symbol.hasInstance]?(realHasInstance=Function.prototype[Symbol.hasInstance],Object.defineProperty(Writable,Symbol.hasInstance,{value:function(e){return!!realHasInstance.call(this,e)||this===Writable&&(e&&e._writableState instanceof WritableState)}})):realHasInstance=function(e){return e instanceof this},Writable.prototype.pipe=function(){errorOrDestroy(this,new ERR_STREAM_CANNOT_PIPE)},Writable.prototype.write=function(e,t,r){var i=this._writableState,n=!1,o=!i.objectMode&&_isUint8Array(e);return o&&!Buffer.isBuffer(e)&&(e=_uint8ArrayToBuffer(e)),"function"==typeof t&&(r=t,t=null),o?t="buffer":t||(t=i.defaultEncoding),"function"!=typeof r&&(r=nop),i.ending?writeAfterEnd(this,r):(o||validChunk(this,i,e,r))&&(i.pendingcb++,n=writeOrBuffer(this,i,o,e,t,r)),n},Writable.prototype.cork=function(){this._writableState.corked++},Writable.prototype.uncork=function(){var e=this._writableState;e.corked&&(e.corked--,e.writing||e.corked||e.bufferProcessing||!e.bufferedRequest||clearBuffer(this,e))},Writable.prototype.setDefaultEncoding=function(e){if("string"==typeof e&&(e=e.toLowerCase()),!(["hex","utf8","utf-8","ascii","binary","base64","ucs2","ucs-2","utf16le","utf-16le","raw"].indexOf((e+"").toLowerCase())>-1))throw new ERR_UNKNOWN_ENCODING(e);return this._writableState.defaultEncoding=e,this},Object.defineProperty(Writable.prototype,"writableBuffer",{enumerable:!1,get:function(){return this._writableState&&this._writableState.getBuffer()}}),Object.defineProperty(Writable.prototype,"writableHighWaterMark",{enumerable:!1,get:function(){return this._writableState.highWaterMark}}),Writable.prototype._write=function(e,t,r){r(new ERR_METHOD_NOT_IMPLEMENTED("_write()"))},Writable.prototype._writev=null,Writable.prototype.end=function(e,t,r){var i=this._writableState;return"function"==typeof e?(r=e,e=null,t=null):"function"==typeof t&&(r=t,t=null),null!=e&&this.write(e,t),i.corked&&(i.corked=1,this.uncork()),i.ending||endWritable(this,i,r),this},Object.defineProperty(Writable.prototype,"writableLength",{enumerable:!1,get:function(){return this._writableState.length}}),Object.defineProperty(Writable.prototype,"destroyed",{enumerable:!1,get:function(){return void 0!==this._writableState&&this._writableState.destroyed},set:function(e){this._writableState&&(this._writableState.destroyed=e)}}),Writable.prototype.destroy=destroyImpl.destroy,Writable.prototype._undestroy=destroyImpl.undestroy,Writable.prototype._destroy=function(e,t){t(e)};

}).call(this)}).call(this,require('_process'),typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"../errors":42,"./_stream_duplex":43,"./internal/streams/destroy":50,"./internal/streams/state":54,"./internal/streams/stream":55,"_process":16,"buffer":10,"inherits":15,"util-deprecate":62}],48:[function(require,module,exports){
(function (process){(function (){
"use strict";var _Object$setPrototypeO;function _defineProperty(e,t,r){return t in e?Object.defineProperty(e,t,{value:r,enumerable:!0,configurable:!0,writable:!0}):e[t]=r,e}var finished=require("./end-of-stream"),kLastResolve=Symbol("lastResolve"),kLastReject=Symbol("lastReject"),kError=Symbol("error"),kEnded=Symbol("ended"),kLastPromise=Symbol("lastPromise"),kHandlePromise=Symbol("handlePromise"),kStream=Symbol("stream");function createIterResult(e,t){return{value:e,done:t}}function readAndResolve(e){var t=e[kLastResolve];if(null!==t){var r=e[kStream].read();null!==r&&(e[kLastPromise]=null,e[kLastResolve]=null,e[kLastReject]=null,t(createIterResult(r,!1)))}}function onReadable(e){process.nextTick(readAndResolve,e)}function wrapForNext(e,t){return function(r,o){e.then(function(){t[kEnded]?r(createIterResult(void 0,!0)):t[kHandlePromise](r,o)},o)}}var AsyncIteratorPrototype=Object.getPrototypeOf(function(){}),ReadableStreamAsyncIteratorPrototype=Object.setPrototypeOf((_defineProperty(_Object$setPrototypeO={get stream(){return this[kStream]},next:function(){var e=this,t=this[kError];if(null!==t)return Promise.reject(t);if(this[kEnded])return Promise.resolve(createIterResult(void 0,!0));if(this[kStream].destroyed)return new Promise(function(t,r){process.nextTick(function(){e[kError]?r(e[kError]):t(createIterResult(void 0,!0))})});var r,o=this[kLastPromise];if(o)r=new Promise(wrapForNext(o,this));else{var n=this[kStream].read();if(null!==n)return Promise.resolve(createIterResult(n,!1));r=new Promise(this[kHandlePromise])}return this[kLastPromise]=r,r}},Symbol.asyncIterator,function(){return this}),_defineProperty(_Object$setPrototypeO,"return",function(){var e=this;return new Promise(function(t,r){e[kStream].destroy(null,function(e){e?r(e):t(createIterResult(void 0,!0))})})}),_Object$setPrototypeO),AsyncIteratorPrototype),createReadableStreamAsyncIterator=function(e){var t,r=Object.create(ReadableStreamAsyncIteratorPrototype,(_defineProperty(t={},kStream,{value:e,writable:!0}),_defineProperty(t,kLastResolve,{value:null,writable:!0}),_defineProperty(t,kLastReject,{value:null,writable:!0}),_defineProperty(t,kError,{value:null,writable:!0}),_defineProperty(t,kEnded,{value:e._readableState.endEmitted,writable:!0}),_defineProperty(t,kHandlePromise,{value:function(e,t){var o=r[kStream].read();o?(r[kLastPromise]=null,r[kLastResolve]=null,r[kLastReject]=null,e(createIterResult(o,!1))):(r[kLastResolve]=e,r[kLastReject]=t)},writable:!0}),t));return r[kLastPromise]=null,finished(e,function(e){if(e&&"ERR_STREAM_PREMATURE_CLOSE"!==e.code){var t=r[kLastReject];return null!==t&&(r[kLastPromise]=null,r[kLastResolve]=null,r[kLastReject]=null,t(e)),void(r[kError]=e)}var o=r[kLastResolve];null!==o&&(r[kLastPromise]=null,r[kLastResolve]=null,r[kLastReject]=null,o(createIterResult(void 0,!0))),r[kEnded]=!0}),e.on("readable",onReadable.bind(null,r)),r};module.exports=createReadableStreamAsyncIterator;

}).call(this)}).call(this,require('_process'))
},{"./end-of-stream":51,"_process":16}],49:[function(require,module,exports){
"use strict";function ownKeys(e,t){var n=Object.keys(e);if(Object.getOwnPropertySymbols){var r=Object.getOwnPropertySymbols(e);t&&(r=r.filter(function(t){return Object.getOwnPropertyDescriptor(e,t).enumerable})),n.push.apply(n,r)}return n}function _objectSpread(e){for(var t=1;t<arguments.length;t++){var n=null!=arguments[t]?arguments[t]:{};t%2?ownKeys(Object(n),!0).forEach(function(t){_defineProperty(e,t,n[t])}):Object.getOwnPropertyDescriptors?Object.defineProperties(e,Object.getOwnPropertyDescriptors(n)):ownKeys(Object(n)).forEach(function(t){Object.defineProperty(e,t,Object.getOwnPropertyDescriptor(n,t))})}return e}function _defineProperty(e,t,n){return t in e?Object.defineProperty(e,t,{value:n,enumerable:!0,configurable:!0,writable:!0}):e[t]=n,e}function _classCallCheck(e,t){if(!(e instanceof t))throw new TypeError("Cannot call a class as a function")}function _defineProperties(e,t){for(var n=0;n<t.length;n++){var r=t[n];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(e,r.key,r)}}function _createClass(e,t,n){return t&&_defineProperties(e.prototype,t),n&&_defineProperties(e,n),e}var _require=require("buffer"),Buffer=_require.Buffer,_require2=require("util"),inspect=_require2.inspect,custom=inspect&&inspect.custom||"inspect";function copyBuffer(e,t,n){Buffer.prototype.copy.call(e,t,n)}module.exports=function(){function e(){_classCallCheck(this,e),this.head=null,this.tail=null,this.length=0}return _createClass(e,[{key:"push",value:function(e){var t={data:e,next:null};this.length>0?this.tail.next=t:this.head=t,this.tail=t,++this.length}},{key:"unshift",value:function(e){var t={data:e,next:this.head};0===this.length&&(this.tail=t),this.head=t,++this.length}},{key:"shift",value:function(){if(0!==this.length){var e=this.head.data;return 1===this.length?this.head=this.tail=null:this.head=this.head.next,--this.length,e}}},{key:"clear",value:function(){this.head=this.tail=null,this.length=0}},{key:"join",value:function(e){if(0===this.length)return"";for(var t=this.head,n=""+t.data;t=t.next;)n+=e+t.data;return n}},{key:"concat",value:function(e){if(0===this.length)return Buffer.alloc(0);for(var t=Buffer.allocUnsafe(e>>>0),n=this.head,r=0;n;)copyBuffer(n.data,t,r),r+=n.data.length,n=n.next;return t}},{key:"consume",value:function(e,t){var n;return e<this.head.data.length?(n=this.head.data.slice(0,e),this.head.data=this.head.data.slice(e)):n=e===this.head.data.length?this.shift():t?this._getString(e):this._getBuffer(e),n}},{key:"first",value:function(){return this.head.data}},{key:"_getString",value:function(e){var t=this.head,n=1,r=t.data;for(e-=r.length;t=t.next;){var a=t.data,i=e>a.length?a.length:e;if(i===a.length?r+=a:r+=a.slice(0,e),0===(e-=i)){i===a.length?(++n,t.next?this.head=t.next:this.head=this.tail=null):(this.head=t,t.data=a.slice(i));break}++n}return this.length-=n,r}},{key:"_getBuffer",value:function(e){var t=Buffer.allocUnsafe(e),n=this.head,r=1;for(n.data.copy(t),e-=n.data.length;n=n.next;){var a=n.data,i=e>a.length?a.length:e;if(a.copy(t,t.length-e,0,i),0===(e-=i)){i===a.length?(++r,n.next?this.head=n.next:this.head=this.tail=null):(this.head=n,n.data=a.slice(i));break}++r}return this.length-=r,t}},{key:custom,value:function(e,t){return inspect(this,_objectSpread({},t,{depth:0,customInspect:!1}))}}]),e}();

},{"buffer":10,"util":9}],50:[function(require,module,exports){
(function (process){(function (){
"use strict";function destroy(t,e){var r=this,i=this._readableState&&this._readableState.destroyed,a=this._writableState&&this._writableState.destroyed;return i||a?(e?e(t):t&&(this._writableState?this._writableState.errorEmitted||(this._writableState.errorEmitted=!0,process.nextTick(emitErrorNT,this,t)):process.nextTick(emitErrorNT,this,t)),this):(this._readableState&&(this._readableState.destroyed=!0),this._writableState&&(this._writableState.destroyed=!0),this._destroy(t||null,function(t){!e&&t?r._writableState?r._writableState.errorEmitted?process.nextTick(emitCloseNT,r):(r._writableState.errorEmitted=!0,process.nextTick(emitErrorAndCloseNT,r,t)):process.nextTick(emitErrorAndCloseNT,r,t):e?(process.nextTick(emitCloseNT,r),e(t)):process.nextTick(emitCloseNT,r)}),this)}function emitErrorAndCloseNT(t,e){emitErrorNT(t,e),emitCloseNT(t)}function emitCloseNT(t){t._writableState&&!t._writableState.emitClose||t._readableState&&!t._readableState.emitClose||t.emit("close")}function undestroy(){this._readableState&&(this._readableState.destroyed=!1,this._readableState.reading=!1,this._readableState.ended=!1,this._readableState.endEmitted=!1),this._writableState&&(this._writableState.destroyed=!1,this._writableState.ended=!1,this._writableState.ending=!1,this._writableState.finalCalled=!1,this._writableState.prefinished=!1,this._writableState.finished=!1,this._writableState.errorEmitted=!1)}function emitErrorNT(t,e){t.emit("error",e)}function errorOrDestroy(t,e){var r=t._readableState,i=t._writableState;r&&r.autoDestroy||i&&i.autoDestroy?t.destroy(e):t.emit("error",e)}module.exports={destroy:destroy,undestroy:undestroy,errorOrDestroy:errorOrDestroy};
...
},{"events":12}],56:[function(require,module,exports){
...
function streamToBlob(e,o){if(null!=o&&"string"!=typeof o)throw new Error("Invalid mimetype, expected string.");return new Promise((n,r)=>{const t=[];e.on("data",e=>t.push(e)).once("end",()=>{const e=null!=o?new Blob(t,{type:o}):new Blob(t);n(e)}).once("error",r)})}module.exports=streamToBlob;
...
const utils=require("./utils"),qs=require("querystring"),{parseTimestamp:parseTimestamp}=require("m3u8stream"),BASE_URL="https://www.youtube.com/watch?v=",TITLE_TO_CATEGORY={song:{name:"Music",url:"https://music.youtube.com/"}},getText=e=>e?e.runs?e.runs[0].text:e.simpleText:null;exports.getMedia=(e=>{let t={},r=[];try{r=e.response.contents.twoColumnWatchNextResults.results.results.contents}catch(e){}let a=r.find(e=>e.videoSecondaryInfoRenderer);if(!a)return{};try{let e=(a.metadataRowContainer||a.videoSecondaryInfoRenderer.metadataRowContainer).metadataRowContainerRenderer.rows;for(let r of e)if(r.metadataRowRenderer){let e=getText(r.metadataRowRenderer.title).toLowerCase(),a=r.metadataRowRenderer.contents[0];t[e]=getText(a);let n=a.runs;n&&n[0].navigationEndpoint&&(t[`${e}_url`]=new URL(n[0].navigationEndpoint.commandMetadata.webCommandMetadata.url,BASE_URL).toString()),e in TITLE_TO_CATEGORY&&(t.category=TITLE_TO_CATEGORY[e].name,t.category_url=TITLE_TO_CATEGORY[e].url)}else if(r.richMetadataRowRenderer){let e=r.richMetadataRowRenderer.contents,a=e.filter(e=>"RICH_METADATA_RENDERER_STYLE_BOX_ART"===e.richMetadataRenderer.style);for(let{richMetadataRenderer:e}of a){let r=e;t.year=getText(r.subtitle);let a=getText(r.callToAction).split(" ")[1];t[a]=getText(r.title),t[`${a}_url`]=new URL(r.endpoint.commandMetadata.webCommandMetadata.url,BASE_URL).toString(),t.thumbnails=r.thumbnail.thumbnails}let n=e.filter(e=>"RICH_METADATA_RENDERER_STYLE_TOPIC"===e.richMetadataRenderer.style);for(let{richMetadataRenderer:e}of n){let r=e;t.category=getText(r.title),t.category_url=new URL(r.endpoint.commandMetadata.webCommandMetadata.url,BASE_URL).toString()}}}catch(e){}return t});const isVerified=e=>!(!e||!e.find(e=>"Verified"===e.metadataBadgeRenderer.tooltip));exports.getAuthor=(e=>{let t,r,a=[],n=!1;try{let o=e.response.contents.twoColumnWatchNextResults.results.results.contents.find(e=>e.videoSecondaryInfoRenderer&&e.videoSecondaryInfoRenderer.owner&&e.videoSecondaryInfoRenderer.owner.videoOwnerRenderer).videoSecondaryInfoRenderer.owner.videoOwnerRenderer;t=o.navigationEndpoint.browseEndpoint.browseId,a=o.thumbnail.thumbnails.map(e=>(e.url=new URL(e.url,BASE_URL).toString(),e)),r=utils.parseAbbreviatedNumber(getText(o.subscriberCountText)),n=isVerified(o.badges)}catch(e){}try{let o=e.player_response.microformat&&e.player_response.microformat.playerMicroformatRenderer,s=o&&o.channelId||t||e.player_response.videoDetails.channelId,i={id:s,name:o?o.ownerChannelName:e.player_response.videoDetails.author,user:o?o.ownerProfileUrl.split("/").slice(-1)[0]:null,channel_url:`https://www.youtube.com/channel/${s}`,external_channel_url:o?`https://www.youtube.com/channel/${o.externalChannelId}`:"",user_url:o?new URL(o.ownerProfileUrl,BASE_URL).toString():"",thumbnails:a,verified:n,subscriber_count:r};return a.length&&utils.deprecate(i,"avatar",i.thumbnails[0].url,"author.avatar","author.thumbnails[0].url"),i}catch(e){return{}}});const parseRelatedVideo=(e,t)=>{if(e)try{let r=getText(e.viewCountText),a=getText(e.shortViewCountText),n=t.find(t=>t.id===e.videoId);/^\d/.test(a)||(a=n&&n.short_view_count_text||""),r=(/^\d/.test(r)?r:a).split(" ")[0];let o=e.shortBylineText.runs[0].navigationEndpoint.browseEndpoint,s=o.browseId,i=getText(e.shortBylineText),l=(o.canonicalBaseUrl||"").split("/").slice(-1)[0],d={id:e.videoId,title:getText(e.title),published:getText(e.publishedTimeText),author:{id:s,name:i,user:l,channel_url:`https://www.youtube.com/channel/${s}`,user_url:`https://www.youtube.com/user/${l}`,thumbnails:e.channelThumbnail.thumbnails.map(e=>(e.url=new URL(e.url,BASE_URL).toString(),e)),verified:isVerified(e.ownerBadges),[Symbol.toPrimitive]:()=>(console.warn("`relatedVideo.author` will be removed in a near future release, use `relatedVideo.author.name` instead."),d.author.name)},short_view_count_text:a.split(" ")[0],view_count:r.replace(/,/g,""),length_seconds:e.lengthText?Math.floor(parseTimestamp(getText(e.lengthText))/1e3):t&&`${t.length_seconds}`,thumbnails:e.thumbnail.thumbnails,richThumbnails:e.richThumbnail?e.richThumbnail.movingThumbnailRenderer.movingThumbnailDetails.thumbnails:[],isLive:!(!e.badges||!e.badges.find(e=>"LIVE NOW"===e.metadataBadgeRenderer.label))};return utils.deprecate(d,"author_thumbnail",d.author.thumbnails[0].url,"relatedVideo.author_thumbnail","relatedVideo.author.thumbnails[0].url"),utils.deprecate(d,"ucid",d.author.id,"relatedVideo.ucid","relatedVideo.author.id"),utils.deprecate(d,"video_thumbnail",d.thumbnails[0].url,"relatedVideo.video_thumbnail","relatedVideo.thumbnails[0].url"),d}catch(e){}};exports.getRelatedVideos=(e=>{let t=[],r=[];try{t=e.response.webWatchNextResponseExtensionData.relatedVideoArgs.split(",").map(e=>qs.parse(e))}catch(e){}try{r=e.response.contents.twoColumnWatchNextResults.secondaryResults.secondaryResults.results}catch(e){return[]}let a=[];for(let e of r||[]){let r=e.compactVideoRenderer;if(r){let e=parseRelatedVideo(r,t);e&&a.push(e)}else{let r=e.compactAutoplayRenderer||e.itemSectionRenderer;if(!r||!Array.isArray(r.contents))continue;for(let e of r.contents){let r=parseRelatedVideo(e.compactVideoRenderer,t);r&&a.push(r)}}}return a}),exports.getLikes=(e=>{try{let t=e.response.contents.twoColumnWatchNextResults.results.results.contents.find(e=>e.videoPrimaryInfoRenderer).videoPrimaryInfoRenderer.videoActions.menuRenderer.topLevelButtons.find(e=>e.toggleButtonRenderer&&"LIKE"===e.toggleButtonRenderer.defaultIcon.iconType);return parseInt(t.toggleButtonRenderer.defaultText.accessibility.accessibilityData.label.replace(/\D+/g,""))}catch(e){return null}}),exports.getDislikes=(e=>{try{let t=e.response.contents.twoColumnWatchNextResults.results.results.contents.find(e=>e.videoPrimaryInfoRenderer).videoPrimaryInfoRenderer.videoActions.menuRenderer.topLevelButtons.find(e=>e.toggleButtonRenderer&&"DISLIKE"===e.toggleButtonRenderer.defaultIcon.iconType);return parseInt(t.toggleButtonRenderer.defaultText.accessibility.accessibilityData.label.replace(/\D+/g,""))}catch(e){return null}}),exports.cleanVideoDetails=((e,t)=>(e.thumbnails=e.thumbnail.thumbnails,delete e.thumbnail,utils.deprecate(e,"thumbnail",{thumbnails:e.thumbnails},"videoDetails.thumbnail.thumbnails","videoDetails.thumbnails"),e.description=e.shortDescription||getText(e.description),delete e.shortDescription,utils.deprecate(e,"shortDescription",e.description,"videoDetails.shortDescription","videoDetails.description"),e.lengthSeconds=t.player_response.microformat&&t.player_response.microformat.playerMicroformatRenderer.lengthSeconds,e)),exports.getStoryboards=(e=>{const t=e.player_response.storyboards&&e.player_response.storyboards.playerStoryboardSpecRenderer&&e.player_response.storyboards.playerStoryboardSpecRenderer.spec&&e.player_response.storyboards.playerStoryboardSpecRenderer.spec.split("|");if(!t)return[];const r=new URL(t.shift());return t.map((e,t)=>{let[a,n,o,s,i,l,d,u]=e.split("#");r.searchParams.set("sigh",u),o=parseInt(o,10),s=parseInt(s,10),i=parseInt(i,10);const c=Math.ceil(o/(s*i));return{templateUrl:r.toString().replace("$L",t).replace("$N",d),thumbnailWidth:parseInt(a,10),thumbnailHeight:parseInt(n,10),thumbnailCount:o,interval:parseInt(l,10),columns:s,rows:i,storyboardCount:c}})}),exports.getChapters=(e=>{const t=e.response&&e.response.playerOverlays&&e.response.playerOverlays.playerOverlayRenderer,r=t&&t.decoratedPlayerBarRenderer&&t.decoratedPlayerBarRenderer.decoratedPlayerBarRenderer&&t.decoratedPlayerBarRenderer.decoratedPlayerBarRenderer.playerBar,a=r&&r.multiMarkersPlayerBarRenderer&&r.multiMarkersPlayerBarRenderer.markersMap,n=Array.isArray(a)&&a.find(e=>e.value&&Array.isArray(e.value.chapters));return n?n.value.chapters.map(e=>({title:getText(e.chapterRenderer.title),start_time:e.chapterRenderer.timeRangeStartMillis/1e3})):[]});

},{"./utils":72,"m3u8stream":2,"querystring":20}],69:[function(require,module,exports){
const querystring=require("querystring"),sax=require("sax"),miniget=require("miniget"),utils=require("./utils"),{setTimeout:setTimeout}=require("timers"),formatUtils=require("./format-utils"),urlUtils=require("./url-utils"),extras=require("./info-extras"),sig=require("./sig"),Cache=require("./cache"),BASE_URL="https://www.youtube.com/watch?v=";exports.cache=new Cache,exports.cookieCache=new Cache(864e5),exports.watchPageCache=new Cache;class UnrecoverableError extends Error{}const AGE_RESTRICTED_URLS=["support.google.com/youtube/?p=age_restrictions","youtube.com/t/community_guidelines"];exports.getBasicInfo=(async(e,t)=>{const r=Object.assign({},miniget.defaultOptions,t.requestOptions);t.requestOptions=Object.assign({},t.requestOptions,{}),t.requestOptions.headers=Object.assign({},{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Safari/537.36"},t.requestOptions.headers);let s=await pipeline([e,t],e=>{let t=utils.playError(e.player_response,["ERROR"],UnrecoverableError),r=privateVideoError(e.player_response);if(t||r)throw t||r;return e&&e.player_response&&(e.player_response.streamingData||isRental(e.player_response)||isNotYetBroadcasted(e.player_response))},r,[getWatchHTMLPage,getWatchJSONPage,getVideoInfoPage]);Object.assign(s,{formats:parseFormats(s.player_response),related_videos:extras.getRelatedVideos(s)});const a=extras.getMedia(s),o={author:extras.getAuthor(s),media:a,likes:extras.getLikes(s),dislikes:extras.getDislikes(s),age_restricted:!!(a&&a.notice_url&&AGE_RESTRICTED_URLS.some(e=>a.notice_url.includes(e))),video_url:BASE_URL+e,storyboards:extras.getStoryboards(s),chapters:extras.getChapters(s)};return s.videoDetails=extras.cleanVideoDetails(Object.assign({},s.player_response&&s.player_response.microformat&&s.player_response.microformat.playerMicroformatRenderer,s.player_response&&s.player_response.videoDetails,o),s),s});const privateVideoError=e=>{let t=e&&e.playabilityStatus;return t&&"LOGIN_REQUIRED"===t.status&&t.messages&&t.messages.filter(e=>/This is a private video/.test(e)).length?new UnrecoverableError(t.reason||t.messages&&t.messages[0]):null},isRental=e=>{let t=e.playabilityStatus;return t&&"UNPLAYABLE"===t.status&&t.errorScreen&&t.errorScreen.playerLegacyDesktopYpcOfferRenderer},isNotYetBroadcasted=e=>{let t=e.playabilityStatus;return t&&"LIVE_STREAM_OFFLINE"===t.status},getWatchHTMLURL=(e,t)=>`${BASE_URL+e}&hl=${t.lang||"en"}`,getWatchHTMLPageBody=(e,t)=>{const r=getWatchHTMLURL(e,t);return exports.watchPageCache.getOrSet(r,()=>utils.exposedMiniget(r,t).text())},EMBED_URL="https://www.youtube.com/embed/",getEmbedPageBody=(e,t)=>{const r=`${EMBED_URL+e}?hl=${t.lang||"en"}`;return utils.exposedMiniget(r,t).text()},getHTML5player=e=>{let t=/<script\s+src="([^"]+)"(?:\s+type="text\/javascript")?\s+name="player_ias\/base"\s*>|"jsUrl":"([^"]+)"/.exec(e);return t?t[1]||t[2]:null},getIdentityToken=(e,t,r,s)=>exports.cookieCache.getOrSet(r,async()=>{let r=(await getWatchHTMLPageBody(e,t)).match(/(["'])ID_TOKEN\1[:,]\s?"([^"]+)"/);if(!r&&s)throw new UnrecoverableError("Cookie header used in request, but unable to find YouTube identity token");return r&&r[2]}),pipeline=async(e,t,r,s)=>{let a;for(let o of s)try{const i=await retryFunc(o,e.concat([a]),r);if(i.player_response&&(i.player_response.videoDetails=assign(a&&a.player_response&&a.player_response.videoDetails,i.player_response.videoDetails),i.player_response=assign(a&&a.player_response,i.player_response)),t(a=assign(a,i),!1))break}catch(e){if(e instanceof UnrecoverableError||o===s[s.length-1])throw e}return a},assign=(e,t)=>{if(!e||!t)return e||t;for(let[r,s]of Object.entries(t))null!=s&&(e[r]=s);return e},retryFunc=async(e,t,r)=>{let s,a=0;for(;a<=r.maxRetries;)try{s=await e(...t);break}catch(e){if(e instanceof UnrecoverableError||e instanceof miniget.MinigetError&&e.statusCode<500||a>=r.maxRetries)throw e;let t=Math.min(++a*r.backoff.inc,r.backoff.max);await new Promise(e=>setTimeout(e,t))}return s},jsonClosingChars=/^[)\]}'\s]+/,parseJSON=(e,t,r)=>{if(!r||"object"==typeof r)return r;try{return r=r.replace(jsonClosingChars,""),JSON.parse(r)}catch(r){throw Error(`Error parsing ${t} in ${e}: ${r.message}`)}},findJSON=(e,t,r,s,a,o)=>{let i=utils.between(r,s,a);if(!i)throw Error(`Could not find ${t} in ${e}`);return parseJSON(e,t,utils.cutAfterJSON(`${o}${i}`))},findPlayerResponse=(e,t)=>{const r=t&&(t.args&&t.args.player_response||t.player_response||t.playerResponse||t.embedded_player_response);return parseJSON(e,"player_response",r)},getWatchJSONURL=(e,t)=>`${getWatchHTMLURL(e,t)}&pbj=1`,getWatchJSONPage=async(e,t)=>{const r=Object.assign({headers:{}},t.requestOptions);let s=r.headers.Cookie||r.headers.cookie;r.headers=Object.assign({"x-youtube-client-name":"1","x-youtube-client-version":"2.20201203.06.00","x-youtube-identity-token":exports.cookieCache.get(s||"browser")||""},r.headers);const a=async(s,a)=>{r.headers["x-youtube-identity-token"]||(r.headers["x-youtube-identity-token"]=await getIdentityToken(e,t,s,a))};s&&await a(s,!0);const o=getWatchJSONURL(e,t),i=await utils.exposedMiniget(o,t,r).text();let n=parseJSON("watch.json","body",i);if("now"===n.reload&&await a("browser",!1),"now"===n.reload||!Array.isArray(n))throw Error("Unable to retrieve video metadata in watch.json");let l=n.reduce((e,t)=>Object.assign(t,e),{});return l.player_response=findPlayerResponse("watch.json",l),l.html5player=l.player&&l.player.assets&&l.player.assets.js,l},getWatchHTMLPage=async(e,t)=>{let r=await getWatchHTMLPageBody(e,t),s={page:"watch"};try{s.player_response=findJSON("watch.html","player_response",r,/\bytInitialPlayerResponse\s*=\s*\{/i,"\n","{")}catch(e){let t=findJSON("watch.html","player_response",r,/\bytplayer\.config\s*=\s*{/,"<\/script>","{");s.player_response=findPlayerResponse("watch.html",t)}return s.response=findJSON("watch.html","response",r,/\bytInitialData("\])?\s*=\s*\{/i,"\n","{"),s.html5player=getHTML5player(r),s},INFO_HOST="www.youtube.com",INFO_PATH="/get_video_info",VIDEO_EURL="https://youtube.googleapis.com/v/",getVideoInfoPage=async(e,t)=>{const r=new URL(`https://${INFO_HOST}${INFO_PATH}`);r.searchParams.set("video_id",e),r.searchParams.set("eurl",VIDEO_EURL+e),r.searchParams.set("ps","default"),r.searchParams.set("gl","US"),r.searchParams.set("hl",t.lang||"en"),r.searchParams.set("html5","1");const s=await utils.exposedMiniget(r.toString(),t).text();let a=querystring.parse(s);return a.player_response=findPlayerResponse("get_video_info",a),a},parseFormats=e=>{let t=[];return e&&e.streamingData&&(t=t.concat(e.streamingData.formats||[]).concat(e.streamingData.adaptiveFormats||[])),t};exports.getInfo=(async(e,t)=>{let r=await exports.getBasicInfo(e,t);const s=r.player_response&&r.player_response.streamingData&&(r.player_response.streamingData.dashManifestUrl||r.player_response.streamingData.hlsManifestUrl);let a=[];if(r.formats.length){if(r.html5player=r.html5player||getHTML5player(await getWatchHTMLPageBody(e,t))||getHTML5player(await getEmbedPageBody(e,t)),!r.html5player)throw Error("Unable to find html5player file");const s=new URL(r.html5player,BASE_URL).toString();a.push(sig.decipherFormats(r.formats,s,t))}if(s&&r.player_response.streamingData.dashManifestUrl){let e=r.player_response.streamingData.dashManifestUrl;a.push(getDashManifest(e,t))}if(s&&r.player_response.streamingData.hlsManifestUrl){let e=r.player_response.streamingData.hlsManifestUrl;a.push(getM3U8(e,t))}let o=await Promise.all(a);return r.formats=Object.values(Object.assign({},...o)),r.formats=r.formats.map(formatUtils.addFormatMeta),r.formats.sort(formatUtils.sortFormats),r.full=!0,r});const getDashManifest=(e,t)=>new Promise((r,s)=>{let a={};const o=sax.parser(!1);let i;o.onerror=s,o.onopentag=(t=>{if("ADAPTATIONSET"===t.name)i=t.attributes;else if("REPRESENTATION"===t.name){const r=parseInt(t.attributes.ID);isNaN(r)||(a[e]=Object.assign({itag:r,url:e,bitrate:parseInt(t.attributes.BANDWIDTH),mimeType:`${i.MIMETYPE}; codecs="${t.attributes.CODECS}"`},t.attributes.HEIGHT?{width:parseInt(t.attributes.WIDTH),height:parseInt(t.attributes.HEIGHT),fps:parseInt(t.attributes.FRAMERATE)}:{audioSampleRate:t.attributes.AUDIOSAMPLINGRATE}))}}),o.onend=(()=>{r(a)});const n=utils.exposedMiniget(new URL(e,BASE_URL).toString(),t);n.setEncoding("utf8"),n.on("error",s),n.on("data",e=>{o.write(e)}),n.on("end",o.close.bind(o))}),getM3U8=async(e,t)=>{e=new URL(e,BASE_URL);const r=await utils.exposedMiniget(e.toString(),t).text();let s={};return r.split("\n").filter(e=>/^https?:\/\//.test(e)).forEach(e=>{const t=parseInt(e.match(/\/itag\/(\d+)\//)[1]);s[e]={itag:t,url:e}}),s};for(let e of["getBasicInfo","getInfo"]){const t=exports[e];exports[e]=(async(r,s={})=>{utils.checkForUpdates();let a=await urlUtils.getVideoID(r);const o=[e,a,s.lang].join("-");return exports.cache.getOrSet(o,()=>t(a,s))})}exports.validateID=urlUtils.validateID,exports.validateURL=urlUtils.validateURL,exports.getURLVideoID=urlUtils.getURLVideoID,exports.getVideoID=urlUtils.getVideoID;
...
const querystring=require("querystring"),Cache=require("./cache"),utils=require("./utils");exports.cache=new Cache,exports.getTokens=((e,r)=>exports.cache.getOrSet(e,async()=>{const t=await utils.exposedMiniget(e,r).text(),s=exports.extractActions(t);if(!s||!s.length)throw Error("Could not extract signature deciphering actions");return exports.cache.set(e,s),s})),exports.decipher=((e,r)=>{r=r.split("");for(let t=0,s=e.length;t<s;t++){let s,a=e[t];switch(a[0]){case"r":r=r.reverse();break;case"w":s=~~a.slice(1),r=swapHeadAndPosition(r,s);break;case"s":s=~~a.slice(1),r=r.slice(s);break;case"p":s=~~a.slice(1),r.splice(0,s)}}return r.join("")});const swapHeadAndPosition=(e,r)=>{const t=e[0];return e[0]=e[r%e.length],e[r]=t,e},jsVarStr="[a-zA-Z_\\$][a-zA-Z_0-9]*",jsSingleQuoteStr="'[^'\\\\]*(:?\\\\[\\s\\S][^'\\\\]*)*'",jsDoubleQuoteStr='"[^"\\\\]*(:?\\\\[\\s\\S][^"\\\\]*)*"',jsQuoteStr=`(?:${jsSingleQuoteStr}|${jsDoubleQuoteStr})`,jsKeyStr=`(?:${jsVarStr}|${jsQuoteStr})`,jsPropStr=`(?:\\.${jsVarStr}|\\[${jsQuoteStr}\\])`,jsEmptyStr="(?:''|\"\")",reverseStr=":function\\(a\\)\\{(?:return )?a\\.reverse\\(\\)\\}",sliceStr=":function\\(a,b\\)\\{return a\\.slice\\(b\\)\\}",spliceStr=":function\\(a,b\\)\\{a\\.splice\\(0,b\\)\\}",swapStr=":function\\(a,b\\)\\{var c=a\\[0\\];a\\[0\\]=a\\[b(?:%a\\.length)?\\];a\\[b(?:%a\\.length)?\\]=c(?:;return a)?\\}",actionsObjRegexp=new RegExp(`var (${jsVarStr})=\\{((?:(?:${jsKeyStr}${reverseStr}|${jsKeyStr}${sliceStr}|${jsKeyStr}${spliceStr}|${jsKeyStr}${swapStr}),?\\r?\\n?)+)\\};`),actionsFuncRegexp=new RegExp(`${`function(?: ${jsVarStr})?\\(a\\)\\{`+"a=a\\.split\\((?:''|\"\")\\);\\s*"+`((?:(?:a=)?${jsVarStr}`}${jsPropStr}\\(a,\\d+\\);)+)`+"return a\\.join\\((?:''|\"\")\\)\\}"),reverseRegexp=new RegExp(`(?:^|,)(${jsKeyStr})${reverseStr}`,"m"),sliceRegexp=new RegExp(`(?:^|,)(${jsKeyStr})${sliceStr}`,"m"),spliceRegexp=new RegExp(`(?:^|,)(${jsKeyStr})${spliceStr}`,"m"),swapRegexp=new RegExp(`(?:^|,)(${jsKeyStr})${swapStr}`,"m");exports.extractActions=(e=>{const r=actionsObjRegexp.exec(e),t=actionsFuncRegexp.exec(e);if(!r||!t)return null;const s=r[1].replace(/\$/g,"\\$"),a=r[2].replace(/\$/g,"\\$"),c=t[1].replace(/\$/g,"\\$");let n=reverseRegexp.exec(a);const p=n&&n[1].replace(/\$/g,"\\$").replace(/\$|^'|^"|'$|"$/g,""),i=(n=sliceRegexp.exec(a))&&n[1].replace(/\$/g,"\\$").replace(/\$|^'|^"|'$|"$/g,""),o=(n=spliceRegexp.exec(a))&&n[1].replace(/\$/g,"\\$").replace(/\$|^'|^"|'$|"$/g,""),l=(n=swapRegexp.exec(a))&&n[1].replace(/\$/g,"\\$").replace(/\$|^'|^"|'$|"$/g,""),$=`(${[p,i,o,l].join("|")})`,u=new RegExp(`(?:a=)?${s}(?:\\.${$}|\\['${$}'\\]|\\["${$}"\\])`+"\\(a,(\\d+)\\)","g"),g=[];for(;null!==(n=u.exec(c));){switch(n[1]||n[2]||n[3]){case l:g.push(`w${n[4]}`);break;case p:g.push("r");break;case i:g.push(`s${n[4]}`);break;case o:g.push(`p${n[4]}`)}}return g}),exports.setDownloadURL=((e,r)=>{let t;if(!e.url)return;t=e.url;try{t=decodeURIComponent(t)}catch(e){return}const s=new URL(t);s.searchParams.set("ratebypass","yes"),r&&s.searchParams.set(e.sp||"signature",r),e.url=s.toString()}),exports.decipherFormats=(async(e,r,t)=>{let s={},a=await exports.getTokens(r,t);return e.forEach(e=>{let r=e.signatureCipher||e.cipher;r&&(Object.assign(e,querystring.parse(r)),delete e.signatureCipher,delete e.cipher);const t=a&&e.s?exports.decipher(a,e.s):null;exports.setDownloadURL(e,t),s[e.url]=e}),s});

},{"./cache":64,"./utils":72,"querystring":20}],71:[function(require,module,exports){
const validQueryDomains=new Set(["youtube.com","www.youtube.com","m.youtube.com","music.youtube.com","gaming.youtube.com"]),validPathDomains=/^https?:\/\/(youtu\.be\/|(www\.)?youtube.com\/(embed|v|shorts)\/)/;exports.getURLVideoID=(e=>{const t=new URL(e);let o=t.searchParams.get("v");if(validPathDomains.test(e)&&!o){const e=t.pathname.split("/");o=e[e.length-1]}else if(t.hostname&&!validQueryDomains.has(t.hostname))throw Error("Not a YouTube domain");if(!o)throw Error(`No video id found: ${e}`);if(o=o.substring(0,11),!exports.validateID(o))throw TypeError(`Video id (${o}) does not match expected `+`format (${idRegex.toString()})`);return o});const urlRegex=/^https?:\/\//;exports.getVideoID=(e=>{if(exports.validateID(e))return e;if(urlRegex.test(e))return exports.getURLVideoID(e);throw Error(`No video id found: ${e}`)});const idRegex=/^[a-zA-Z0-9-_]{11}$/;exports.validateID=(e=>idRegex.test(e)),exports.validateURL=(e=>{try{return exports.getURLVideoID(e),!0}catch(e){return!1}});

},{}],72:[function(require,module,exports){
(function (process){(function (){
const miniget=require("miniget");exports.between=((e,t,r)=>{let s;if(t instanceof RegExp){const r=e.match(t);if(!r)return"";s=r.index+r[0].length}else{if(-1===(s=e.indexOf(t)))return"";s+=t.length}return-1===(s=(e=e.slice(s)).indexOf(r))?"":e=e.slice(0,s)}),exports.parseAbbreviatedNumber=(e=>{const t=e.replace(",",".").replace(" ","").match(/([\d,.]+)([MK]?)/);if(t){let[,e,r]=t;return e=parseFloat(e),Math.round("M"===r?1e6*e:"K"===r?1e3*e:e)}return null}),exports.cutAfterJSON=(e=>{let t,r;if("["===e[0]?(t="[",r="]"):"{"===e[0]&&(t="{",r="}"),!t)throw new Error(`Can't cut unsupported JSON (need to begin with [ or { ) but got: ${e[0]}`);let s,n=!1,o=!1,a=0;for(s=0;s<e.length;s++)if('"'!==e[s]||o){if(o="\\"===e[s]&&!o,!n&&(e[s]===t?a++:e[s]===r&&a--,0===a))return e.substr(0,s+1)}else n=!n;throw Error("Can't cut unsupported JSON (no matching closing bracket found)")}),exports.playError=((e,t,r=Error)=>{let s=e&&e.playabilityStatus;return s&&t.includes(s.status)?new r(s.reason||s.messages&&s.messages[0]):null}),exports.exposedMiniget=((e,t={},r)=>{const s=miniget(e,r||t.requestOptions);return"function"==typeof t.requestCallback&&t.requestCallback(s),s}),exports.deprecate=((e,t,r,s,n)=>{Object.defineProperty(e,t,{get:()=>(console.warn(`\`${s}\` will be removed in a near future release, `+`use \`${n}\` instead.`),r)})});const pkg=require("../package.json"),UPDATE_INTERVAL=432e5;exports.lastUpdateCheck=0,exports.checkForUpdates=(()=>!process.env.YTDL_NO_UPDATE&&!pkg.version.startsWith("0.0.0-")&&Date.now()-exports.lastUpdateCheck>=432e5?(exports.lastUpdateCheck=Date.now(),miniget("https://api.github.com/repos/fent/node-ytdl-core/releases/latest",{headers:{"User-Agent":"ytdl-core"}}).text().then(e=>{JSON.parse(e).tag_name!==`v${pkg.version}`&&console.warn('[33mWARNING:[0m ytdl-core is out of date! Update with "npm install ytdl-core@latest".')},e=>{console.warn("Error checking for updates:",e.message),console.warn("You can disable this check by setting the `YTDL_NO_UPDATE` env variable.")})):null);

}).call(this)}).call(this,require('_process'))
},{"../package.json":74,"_process":16,"miniget":73}],73:[function(require,module,exports){
(function (process){(function (){
"use strict";var __importDefault=this&&this.__importDefault||function(e){return e&&e.__esModule?e:{default:e}};const http_1=__importDefault(require("http")),https_1=__importDefault(require("https")),stream_1=require("stream"),httpLibs={"http:":http_1.default,"https:":https_1.default},redirectStatusCodes=new Set([301,302,303,307,308]),retryStatusCodes=new Set([429,503]),requestEvents=["connect","continue","information","socket","timeout","upgrade"],responseEvents=["aborted"];function Miniget(e,t={}){var r;const o=Object.assign({},Miniget.defaultOptions,t),n=new stream_1.PassThrough({highWaterMark:o.highWaterMark});let s,i,a;n.destroyed=n.aborted=!1;let d,c,u,l=0,m=0,f=0,h=!1,p=0,g=0;if(null===(r=o.headers)||void 0===r?void 0:r.Range){let e=/bytes=(\d+)-(\d+)?/.exec(`${o.headers.Range}`);e&&(p=parseInt(e[1],10),u=parseInt(e[2],10))}o.acceptEncoding&&(o.headers=Object.assign({"Accept-Encoding":Object.keys(o.acceptEncoding).join(", ")},o.headers));const b=e=>"HEAD"!==t.method&&!(()=>!h||g===c)()&&f++<o.maxReconnects&&((e=>{a=null,m=0;let t=o.backoff.inc,r=Math.min(t,o.backoff.max);d=setTimeout(v,r),n.emit("reconnect",f,e)})(e),!0),y=e=>{if(n.destroyed)return!1;if((()=>a&&g>0)())return b(e.err);if((!e.err||"ENOTFOUND"===e.err.message)&&m++<o.maxRetries){let t=e.retryAfter||Math.min(m*o.backoff.inc,o.backoff.max);return d=setTimeout(v,t),n.emit("retry",m,e.err),!0}return!1},M=(e,t)=>{for(let r of t)e.on(r,n.emit.bind(n,r))},v=()=>{let t,r={};try{let o="string"==typeof e?new URL(e):e;r=Object.assign({},{host:o.host,hostname:o.hostname,path:o.pathname+o.search+o.hash,port:o.port,protocol:o.protocol}),o.username&&(r.auth=`${o.username}:${o.password}`),t=httpLibs[String(r.protocol)]}catch(e){}if(!t)return void n.emit("error",new Miniget.MinigetError(`Invalid URL: ${e}`));if(Object.assign(r,o),h&&g>0){let e=g+p,t=u||"";r.headers=Object.assign({},r.headers,{Range:`bytes=${e}-${t}`})}if(o.transform){try{r=o.transform(r)}catch(e){return void n.emit("error",e)}if((!r||r.protocol)&&!(t=httpLibs[String(null==r?void 0:r.protocol)]))return void n.emit("error",new Miniget.MinigetError("Invalid URL object from `transform` function"))}const d=e=>{n.destroyed||n.readableEnded||n._readableState.ended||(f(),y({err:e})?s.removeListener("close",m):n.emit("error",e))},m=()=>{f(),y({})},f=()=>{s.removeListener("close",m),null==i||i.removeListener("data",C),null==a||a.removeListener("end",x)},C=e=>{g+=e.length},x=()=>{f(),b()||n.end()};(s=t.request(r,t=>{if(!n.destroyed)if(redirectStatusCodes.has(t.statusCode)){if(l++>=o.maxRedirects)n.emit("error",new Miniget.MinigetError("Too many redirects"));else{if(!t.headers.location){let e=new Miniget.MinigetError("Redirect status code given with no location",t.statusCode);return n.emit("error",e),void f()}e=t.headers.location,setTimeout(v,1e3*parseInt(t.headers["retry-after"]||"0",10)),n.emit("redirect",e)}f()}else if(retryStatusCodes.has(t.statusCode)){if(!y({retryAfter:parseInt(t.headers["retry-after"]||"0",10)})){let e=new Miniget.MinigetError(`Status code: ${t.statusCode}`,t.statusCode);n.emit("error",e)}f()}else{if(t.statusCode&&(t.statusCode<200||t.statusCode>=400)){let e=new Miniget.MinigetError(`Status code: ${t.statusCode}`,t.statusCode);return t.statusCode>=500?d(e):n.emit("error",e),void f()}if(a=t,o.acceptEncoding&&t.headers["content-encoding"])for(let e of t.headers["content-encoding"].split(", ").reverse()){let t=o.acceptEncoding[e];t&&(a=a.pipe(t())).on("error",d)}c||(c=parseInt(`${t.headers["content-length"]}`,10),h="bytes"===t.headers["accept-ranges"]&&c>0&&o.maxReconnects>0),t.on("data",C),a.on("end",x),a.pipe(n,{end:!h}),i=t,n.emit("response",t),t.on("error",d),M(t,responseEvents)}})).on("error",d),s.on("close",m),M(s,requestEvents),n.destroyed&&_(...E),n.emit("request",s),s.end()};let E;n.abort=(e=>{console.warn("`MinigetStream#abort()` has been deprecated in favor of `MinigetStream#destroy()`"),n.aborted=!0,n.emit("abort"),n.destroy(e)});const _=e=>{s.destroy(e),null==a||a.unpipe(n),null==a||a.destroy(),clearTimeout(d)};return n._destroy=((...e)=>{n.destroyed=!0,s?_(...e):E=e}),n.text=(()=>new Promise((e,t)=>{let r="";n.setEncoding("utf8"),n.on("data",e=>r+=e),n.on("end",()=>e(r)),n.on("error",t)})),process.nextTick(v),n}Miniget.MinigetError=class extends Error{constructor(e,t){super(e),this.statusCode=t}},Miniget.defaultOptions={maxRedirects:10,maxRetries:2,maxReconnects:0,backoff:{inc:100,max:1e4}},module.exports=Miniget;
...
class KeyboardLooper{constructor(e,o){this.loops={},this.keyupListenerFn=(e=>this.keyUp(e)),this.keydownListenerFn=(e=>this.keyDown(e)),this.mousedownListenerFn=(e=>this.mouseDown(e)),this.looper=o,this.speeds=[4/9,8/15,2/3,.8,1,5/4,1.5,15/8,9/4],this.initDefaultKeymap(),this.bind(e)}initDefaultKeymap(){this.keymap={ArrowUp:e=>{const o=e.shiftKey?.001:e.ctrlKey?.1:.01;this.sampler.vol+=o},ArrowDown:e=>{const o=e.shiftKey?.001:e.ctrlKey?.1:.01;this.sampler.vol-=o}};for(const e of"qwertyuiop")this.keymap[e]=(o=>{this.skipTo(e)});for(let e of"QWERTYUIOP")this.keymap[e]=(o=>{e=e.toLowerCase(),this.has(e)&&!this.hasComplete(e)||this.initLoop(e,this.sampler.curPos)});for(const e in"asdfghjkl"){const o="asdfghjkl".charAt(e);this.keymap[o]=(o=>{this.sampler.speed=this.speeds[e]})}this.keymap.z=(e=>this.sampler.togglePitchShift())}has(e){return!!this.loops[e]}hasComplete(e){return!!this.loops[e]&&null!=this.loops[e].end}initLoop(e,o){console.log(`[KeyLoop] '${e}' starts at ${o}`),this.loops[e]={start:o}}finalizeLoop(e,o,s=!0){const t=this.loops[e];t&&t.start?(t.end=o,console.log(`[KeyLoop] '${e}' ${t.start}->${t.end} (${t.end-t.start}s)`),s&&this.setupLoop(e)):console.warn(`[KeyLoop] loop ${e} didn't have a startPos`)}clearLoop(e){console.log(`[KeyLoop] clearing '${e}'`),delete this.loops[e]}setupLoop(e){if(this.has(e)){const o=this.loops[e];this.looper.skip2(e,o.start,o.end-o.start)}else console.warn(`[KeyLoop] ${e} not initialized`)}skipTo(e){this.has(e)?this.sampler.seek(this.loops[e].start):console.warn(`[KeyLoop] ${e} not initialized`)}toggleLoop(e){this.looper.has(e)?this.looper.toggle(e):this.setupLoop(e)}keyUp(e){const o=e.key;"QWERTYUIOP".includes(o)&&this.finalizeLoop(o.toLowerCase(),this.sampler.curPos,!1)}keyDown(e){const o=e.key;this.keymap.hasOwnProperty(o)&&(this.keymap[o](e),e.stopImmediatePropagation())}mouseDown(e){"video"==e.target.tagName.toLowerCase()&&e.stopImmediatePropagation()}bind(e){e&&(this.sampler=e),window.addEventListener("keydown",this.keydownListenerFn,!0),window.addEventListener("click",this.mousedownListenerFn,!0),window.addEventListener("keyup",this.keyupListenerFn,!0)}unbind(){window.removeEventListener("keydown",this.keydownListenerFn,!0),window.removeEventListener("keyup",this.keyupListenerFn,!0),window.removeEventListener("click",this.mousedownListenerFn,!0)}}module.exports=KeyboardLooper;

},{}],76:[function(require,module,exports){
const{rand:rand,irand:irand,CallableInstance:CallableInstance}=require("./utils");class Looper extends CallableInstance{constructor(s){super("call"),this.sampler=s,this.loops={}}has(s){return!!this.loops[s]}add(s,t,a=1e3,i=!0){this.stop(s),this.loops[s]={fn:t,fnDelta:a,playing:i},this.run(s)}stop(s){this.has(s)&&(clearInterval(this.loops[s].id),delete this.loops[s])}isPlaying(s){return!!this.has(s)&&this.loops[s].playing}pause(s){this.has(s)&&(this.loops[s].playing=!1)}play(s){this.has(s)&&(this.loops[s].playing=!0,this.run(s))}toggle(s){this.has(s)&&(this.isPlaying(s)?this.pause(s):this.play(s))}run(s){if(!this.isPlaying(s))return;this.loops[s].fn();let t=this.loops[s].fnDelta||.1;"function"==typeof t&&(t=this.loops[s].fnDelta()),"number"!=typeof t&&(console.log(`[${s}] delta is not a number: stopping`),this.pause(s)),t=Math.max(.1,t),console.log(`[${s}] next: ${t}`),clearInterval(this.loops[s].id),this.loops[s].id=setTimeout(()=>this.run(s),1e3*t)}call(...s){this.add(...s)}stut(s){s<=0?this.pause("stut"):this.add("stut",()=>this.sampler.seekDelta(-s),()=>s)}stutCur(s){const t=this.sampler.curPos;s<=0?this.pause("stut"):this.add("stut",()=>this.sampler.seek(t),()=>s)}stutCurRand(s=.1,t=.2){const a=this.sampler.curPos;s<=0?this.pause("stut"):this.add("stut",()=>this.sampler.seek(a),()=>rand(s,t))}stut2(s,t){null!=t?this.stutCurRand(s,t):this.stutCur(s)}skip(s,t,a,i,e){a=a||t,i=i||Math.abs(t),e=e||i,this.add(s,()=>this.sampler.seekD(t,a),()=>rand(i,e))}skip2(s,t,a,i){i=i||a,this.add(s,()=>this.sampler.seek(t),()=>rand(a,i))}}module.exports=Looper;

},{"./utils":79}],77:[function(require,module,exports){
const ytdl=require("ytdl-core"),s2b=require("stream-to-blob"),YoutubePlayer=require("./youtubeFunctions"),Looper=require("./looper"),KeyboardLooper=require("./keyboard"),VideoPreloader=require("./preloader"),{rand:rand,irand:irand,coin:coin,choose:choose}=require("./utils");window.ytdl=ytdl,window.s2b=s2b,window.y=new YoutubePlayer,window.l=new Looper(window.y),window.k=new KeyboardLooper(window.y,window.l),window.p=new VideoPreloader,Object.assign(window,{rand:rand,irand:irand,coin:coin,choose:choose});const forwardFunc=(o,r)=>{window[r]=((...e)=>o[r](...e))},forwardPlayer=["sp","ps","modSpeed","seek","seekD","seekRand","randSpeed"],forwardLooper=["stut","stut2","skip","skip2"];for(const o of forwardPlayer)forwardFunc(window.y,o);for(const o of forwardLooper)forwardFunc(window.l,o);
...
const ytdl=require("ytdl-core"),s2b=require("stream-to-blob"),{CallableInstance:CallableInstance}=require("./utils");class VideoPreloader extends CallableInstance{constructor(){super("call")}openDb(){const e=window.indexedDB.open("ytSamplerTools",1);e.onversionchange=(()=>{this.db.createObjectStore("ytSamplerTools")}),e.onsuccess=(()=>{this.db=e.result,this.dbTransaction=this.db.transaction(["ytSamplerTools"],IDBTransaction.READ_WRITE),this.db.onerror=(e=>{console.warn("[Preload] Error creating/accessing IndexedDB database"),this.db=void 0})})}getId(e){return new URLSearchParams(new URL(e).search).get("v")}clear(){this.hasStorage&&this.db.deleteObjectStore("ytSamplerTools")}hasStorage(){return!!this.db}isStored(e){return!!this.hasStorage()&&!!window.localStorage.ytSamplerToolsStorage[e]}getStoredBlob(e){if(!this.hasStorage())return!1;this.dbTransaction.objectStore("ytSamplerTools").get(e)}storeBlob(e,o){if(!this.hasStorage())return!1;this.dbTransaction.objectStore("ytSamplerTools").put(o,e)}preload(e,o=!1){e=e||window.location.href,console.log("[Preloading]",e);const t=this.getId(e);if(this.isStored(t))console.log("[Preloading] got it from storage"),this.replaceVideo(this.getStoredBlob(t));else{const r=this.getBlob(e);s2b(r).then(e=>{console.log("[Preloading] got blob:",e),this.replaceVideo(e),o&&(console.log("[Preloading] storing blob",t),this.storeBlob(t,e))})}}replaceVideo(e){const o=document.querySelector("video"),t=!o.paused,r=o.currentTime;o.src=URL.createObjectURL(e),o.currentTime=r,t&&document.querySelector("video").play()}getBlob(e,o=!0){let t=0;const r=ytdl(e);return r.on("progress",(e,o,r)=>{const s=Math.round(o/r*100),a=s-t;if(a>=10){t+=10*Math.floor(a/10);const e=Math.round(o/1e4)/100,l=Math.round(r/1e4)/100;console.log(`[Preloading] ${s}% (${e} of ${l}MB)`)}}),r}call(e){this.preload(e)}}module.exports=VideoPreloader;

},{"./utils":79,"stream-to-blob":57,"ytdl-core":67}],79:[function(require,module,exports){
const rand=(t=0,n=1)=>Math.random()*(n-t)+t,irand=(t=0,n=1)=>Math.round(Math.random()*(n-t)+t),coin=(t=.5)=>Math.random()<t,choose=t=>t[irand(0,t.length-1)];function CallableInstance(t){var n=this.constructor.prototype[t],o=function(){return n.apply(o,arguments)};return Object.setPrototypeOf(o,this.constructor.prototype),Object.getOwnPropertyNames(n).forEach(function(t){Object.defineProperty(o,t,Object.getOwnPropertyDescriptor(n,t))}),o}CallableInstance.prototype=Object.create(Function.prototype),module.exports={rand:rand,irand:irand,coin:coin,choose:choose,CallableInstance:CallableInstance};

},{}],80:[function(require,module,exports){
const{rand:rand,irand:irand}=require("./utils");class YoutubePlayer{get v(){return document.querySelector("video")}get curPos(){return this.v.currentTime}get dur(){return this.v.duration}get speed(){return this.v.playbackRate}get randPos(){return rand(0,this.dur)}seek(e){this.v.currentTime=e}seekRand(){this.seek(this.randPos)}seekDelta(e){this.seek(this.curPos+e)}seekDrand(e=-1,s=1){this.seekDelta(rand(e,s))}seekD(e,s){void 0===s?this.seekDelta(e):this.seekDrand(e,s)}set speed(e){this.v.playbackRate=e<.25?.25:e>4?4:e}modSpeed(e,s){return void 0!==s&&(e=rand(e,s)),this.speed=this.speed*e,this.speed}randSpeed(e=-3,s=3){return this.speed=Math.pow(2,rand(e,s)),this.speed}sp(e,s){return null!=e&&(null!=s?randSpeed(e,s):this.speed=e),this.speed}pitchShift(e=!0){const s=this.v;null!=s.preservesPitch?s.preservesPitch=!e:null!=s.mozPreservesPitch?s.mozPreservesPitch=!e:null!=s.webkitPreservesPitch?s.webkitPreservesPitch=!e:console.warn("[pitchShift] not supported by this browser")}ps(e=!0){this.pitchShift(e)}togglePitchShift(){const e=this.v;null!=e.preservesPitch?e.preservesPitch=!e.preservesPitch:null!=e.mozPreservesPitch?e.mozPreservesPitch=!e.mozPreservesPitch:null!=e.webkitPreservesPitch?e.webkitPreservesPitch=!e.webkitPreservesPitch:console.warn("[pitchShift] not supported by this browser")}get vol(){return this.v.volume}set vol(e){this.v.volume=e}}module.exports=YoutubePlayer;
