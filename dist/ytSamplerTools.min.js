...
"use strict";var __importDefault=this&&this.__importDefault||function(e){return e&&e.__esModule?e:{default:e}};const stream_1=require("stream"),miniget_1=__importDefault(require("miniget")),m3u8_parser_1=__importDefault(require("./m3u8-parser")),dash_mpd_parser_1=__importDefault(require("./dash-mpd-parser")),queue_1=require("./queue"),parse_time_1=require("./parse-time"),supportedParsers={m3u8:m3u8_parser_1.default,"dash-mpd":dash_mpd_parser_1.default};let m3u8stream=(e,t={})=>{const r=new stream_1.PassThrough,n=t.chunkReadahead||3,s=t.liveBuffer||2e4,a=t.requestOptions,u=supportedParsers[t.parser||(/\.mpd$/.test(e)?"dash-mpd":"m3u8")];if(!u)throw TypeError(`parser '${t.parser}' not supported`);let i=0;void 0!==t.begin&&(i="string"==typeof t.begin?parse_time_1.humanStr(t.begin):Math.max(t.begin-s,0));const o=e=>{for(let t of["abort","request","response","redirect","retry","reconnect"])e.on(t,r.emit.bind(r,t))};let l;const d=new queue_1.Queue((e,t)=>{l=e;let n=0;e.on("data",e=>n+=e.length),e.pipe(r,{end:!1}),e.on("end",()=>t(null,n))},{concurrency:1});let m=0,p=0;const _=new queue_1.Queue((t,n)=>{let s=Object.assign({},a);t.range&&(s.headers=Object.assign({},s.headers,{Range:`bytes=${t.range.start}-${t.range.end}`}));let u=miniget_1.default(new URL(t.url,e).toString(),s);u.on("error",n),o(u),d.push(u,(e,s)=>{p+=+s,r.emit("progress",{num:++m,size:s,duration:t.duration,url:t.url},_.total,p),n(null)})},{concurrency:n}),h=e=>{y||(r.emit("error",e),r.end())};let c,g,f,q,b=!0,y=!1,w=!1;const D=e=>{if(l=null,e)h(e);else if(!b&&!y&&!w&&_.tasks.length+_.active<=c){let e=Math.max(0,g-(Date.now()-q));b=!0,f=setTimeout(x,e)}else!y&&!w||_.tasks.length||_.active||r.end()};let T,v,M=0;const x=()=>{q=Date.now(),(T=miniget_1.default(e,a)).on("error",h),o(T);const r=T.pipe(new u(t.id));r.on("starttime",e=>{M||(M=e,"string"==typeof t.begin&&i>=0&&(i+=M))}),r.on("endlist",()=>{w=!0}),r.on("endearly",T.unpipe.bind(T,r));let n=[];const l=e=>{if(!e.init){if(e.seq<=v)return;v=e.seq}i=e.time,_.push(e,D),n.push(e)};let d=[],m=0;r.on("item",e=>{let t=Object.assign({time:M},e);if(i<=t.time)l(t);else for(d.push(t),m+=t.duration;d.length>1&&m-d[0].duration>s;){const e=d.shift();m-=e.duration}M+=t.duration}),r.on("end",()=>{T=null,!n.length&&d.length&&d.forEach(e=>{l(e)}),c=Math.max(1,Math.ceil(.01*n.length)),g=n.reduce((e,t)=>t.duration+e,0),b=!1,D(null)})};return x(),r.end=(()=>{y=!0,d.die(),_.die(),clearTimeout(f),null==T||T.destroy(),null==l||l.destroy(),stream_1.PassThrough.prototype.end.call(r,null)}),r};m3u8stream.parseTimestamp=parse_time_1.humanStr,module.exports=m3u8stream;
